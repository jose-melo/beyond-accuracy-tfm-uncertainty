...
class CatBoostMethod(classical_methods):
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        cat_features = list(
            range(self.n_num_features, self.n_num_features + self.n_cat_features)
        )
        if self.C is None:
            ...
            X_train = np.concatenate(
                [self.N["train"], self.C["train"].astype(str)], axis=1
            )
        ...
        if not self.is_regression:
            ...
            y_pred_val = self.model.predict(X_val)
            self.trlog["best_res"] = (
                mean_squared_error(self.y["val"], y_pred_val, squared=False)
                * self.y_info["std"]
            )
        time_cost = time.time() - tic
        with open(
            ops.join(self.args.save_path, "best-val-{}.pkl".format(self.args.seed)),
            "wb",
        ) as f:
