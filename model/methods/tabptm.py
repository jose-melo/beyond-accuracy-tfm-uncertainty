...
class TabPTMMethod(Method):
    ...
    def data_format(self, is_train=True, N=None, C=None, y=None):
        if is_train:
            ...
            if self.is_regression:
                ...
                self.d_out = 1
                self.centers = prepare_meta_feature_regression(
                    self.N, self.y, self.args
                )
            ...
            if torch.cuda.is_available():
                self.centers = [
                    torch.from_numpy(c.astype(np.float64)).cuda() for c in self.centers
                ]
            else:
                self.centers = [
                    torch.from_numpy(c.astype(np.float64)) for c in self.centers
                ]
            ...
            self.val_loader = DataLoader(
                dataset=valset, batch_size=2048, shuffle=False, num_workers=0
            )
        else:
            ...
            self.test_loader = DataLoader(
                dataset=testset, batch_size=2048, shuffle=False, num_workers=0
            )
    ...
    def construct_model(self, model_config=None):
        ...
        if self.is_regression:
            self.model.load_state_dict(
                torch.load(
                    "model/models/models_tabptm/metaregC-numK16-Reweight-LR0.001-maneucbra-log.pth"
                )["params"]
            )
        else:
            self.model.load_state_dict(
                torch.load(
                    "model/models/models_tabptm/metaclsA-numK32-Reweight-LR0.001-maneucbra-log.pth"
                )["params"]
            )
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        torch.save(
            ...
            osp.join(
                self.args.save_path, "epoch-last-{}.pth".format(str(self.args.seed))
            ),
        )
        return time_cost
    ...
    def predict(self, data, info, model_name):
        ...
        self.model.load_state_dict(
            torch.load(
                osp.join(
                    self.args.save_path,
                    model_name + "-{}.pth".format(str(self.args.seed)),
                )
            )["params"]
        )
        print(
    ...
    def bray_curtis_dist(self, X, centers_c, weights=None):
        if weights != None:
            weighted_numerator = torch.sum(
                weights[None, None, :]
                * torch.abs(X[:, None, :] - centers_c[None, :, :]),
                dim=-1,
            )
            weighted_denominator = torch.sum(
                weights[None, None, :]
                * torch.abs(X[:, None, :] + centers_c[None, :, :]),
                dim=-1,
            )
            ...
            numerator = torch.sum(
                torch.abs(X[:, None, :] - centers_c[None, :, :]), dim=-1
            )
    ...
    def canberra_dist(self, X, centers_c, weights=None):
        if weights != None:
            weighted_numerator = weights[None, None, :] * torch.abs(
                X[:, None, :] - centers_c[None, :, :]
            )
            weighted_denominator = (
                weights[None, None, :]
                * (torch.abs(X[:, None, :]) + torch.abs(centers_c[None, :, :]))
                + 1e-10
            )
    ...
    def get_meta_feature_regression(self, X, y=None, is_training=False):
        distance_list = [
            self.distance[i : i + 3] for i in range(0, len(self.distance), 3)
        ]
        distance_pos_list = []
        label_pos_list = []
        ...
        for distance_type in distance_list:
            ...
            if distance_type == "euc":
                distance_pos_c = torch.cdist(
                    X * torch.sqrt(self.mi), centers_c * torch.sqrt(self.mi), p=2.0
                )
            elif distance_type == "man":
                distance_pos_c = torch.cdist(X * self.mi, centers_c * self.mi, p=1.0)
            elif distance_type == "cos":
                distance_pos_c = 1.0 - torch.mm(
                    F.normalize(X, dim=-1), F.normalize(centers_c, dim=-1).t()
                )
            ...
            if is_training:
                topk_pos = torch.topk(
                    distance_pos_c, min(numK + 1, centers_c.shape[0]), largest=False
                )[0]
                topk_pos_labels = labels_c[
                    torch.topk(
                        distance_pos_c, min(numK + 1, centers_c.shape[0]), largest=False
                    )[1]
                ]
                ...
                if zero_mask.any():
                    topk_pos = torch.where(
                        zero_mask[:, None].expand_as(topk_pos[:, 1:]),
                        topk_pos[:, 1 : 1 + numK],
                        topk_pos[:, :numK],
                    )
                    topk_pos_labels = torch.where(
                        zero_mask[:, None].expand_as(topk_pos_labels[:, 1:]),
                        topk_pos_labels[:, 1 : 1 + numK],
                        topk_pos_labels[:, :numK],
                    )
                else:
                ...
                topk_pos = torch.topk(
                    distance_pos_c, min(numK, centers_c.shape[0]), largest=False
                )[0]
                topk_pos_labels = labels_c[
                    torch.topk(
                        distance_pos_c, min(numK, centers_c.shape[0]), largest=False
            ...
            if topk_pos.shape[1] < numK:
                ...
                topk_pos = torch.cat(
                    [
                        topk_pos,
                        torch.tile(
                            torch.max(topk_pos, axis=1, keepdims=True)[0], [1, num_diff]
                        ),
                    ],
                    axis=-1,
                )
                topk_pos_labels = torch.cat(
                    [
                        topk_pos_labels,
                        torch.tile(topk_pos_labels[:, -1:], [1, num_diff]),
                    ],
                    axis=-1,
                )
        ...
        distance_and_label = torch.cat(
            [torch.stack(distance_pos_list), torch.stack(label_pos_list)], dim=2
        )
    ...
    def get_meta_feature_allclass(self, X, y=None, is_training=False):
        ...
        for distance_type in distance_list:
            ...
            for c_index, centers_c in enumerate(self.centers):
                if distance_type == "euc":
                    ...
                    distance_pos_c = torch.cdist(
                        X * self.mi, centers_c * self.mi, p=1.0
                    )
                elif distance_type == "cos":
                    distance_pos_c = 1.0 - torch.mm(
                        F.normalize(X, dim=-1), F.normalize(centers_c, dim=-1).t()
                ...
                if is_training:
                    topk_pos_c = torch.topk(
                        distance_pos_c, min(numK + 1, centers_c.shape[0]), largest=False
                    )[0]
                    nn_mask_pos = torch.zeros_like(topk_pos_c).masked_fill(
                        torch.unsqueeze(y == c_index, -1), 1
                    )
                    nn_mask_neg = torch.zeros_like(topk_pos_c).masked_fill(
                        torch.unsqueeze(y != c_index, -1), 1
                    )

                    zero_mask = topk_pos_c[:, 0] < 1e-6
                    if zero_mask.any():
                        topk_pos_c = (topk_pos_c * nn_mask_pos)[:, 1:] + (
                            topk_pos_c * nn_mask_neg
                        )[:, : min(numK, centers_c.shape[0] - 1)]
                    else:
                        topk_pos_c = (topk_pos_c * nn_mask_pos)[
                            :, : min(numK, centers_c.shape[0] - 1)
                        ] + (topk_pos_c * nn_mask_neg)[
                            :, : min(numK, centers_c.shape[0] - 1)
                        ]
                ...
                if topk_pos_c.shape[1] < numK:
                    ...
                    topk_pos_c = torch.cat(
                        [
                            topk_pos_c,
                            torch.tile(
                                torch.max(topk_pos_c, axis=1, keepdims=True)[0],
                                [1, num_diff],
                            ),
                        ],
                        axis=-1,
        ...
        X_meta = (
            torch.stack(distance_pos_list).permute([2, 1, 0, 3]).contiguous()
        )  # num_inst x num_class x len(distance_list) x numK
        ...
        for i in range(num_dist):
            ...
            for i in range(num_class):
                subset = (
                    X_meta_dist[:, i, :]
                    .topk(num_per_subset, dim=1, largest=False)
                    .values
                )
        ...
        X_meta = (
            torch.stack(X_meta_dist_list)
            .permute([1, 0, 2])
            .unsqueeze(1)
            .expand(-1, num_class, -1, -1)
        )
        label = (
            torch.stack(label_dist_list)
            .permute([1, 0, 2])
            .unsqueeze(1)
            .expand(-1, num_class, -1, -1)
        )
        ...
        mask_tensor[
            label
            == torch.arange(num_class).cuda().unsqueeze(0).unsqueeze(2).unsqueeze(3)
        ] = 1.0
    ...
    def validate(self, epoch):
        ...
        print(
            "epoch {}, val, loss={:.4f} {} result={:.4f}".format(
                epoch, vl, task_type, vres[0]
            )
        )
        if measure(vres[0], self.trlog["best_res"]) or epoch == 0:
            ...
            torch.save(
                dict(params=self.model.state_dict()),
                osp.join(
                    self.args.save_path, "best-val-{}.pth".format(str(self.args.seed))
                ),
            )
            self.val_count = 0
        else:
