...
from model.utils import Timer, Averager, set_seeds, get_device
    ...
    def construct_model(self, model_config=None):
        ...
        self.model = GRANDE(
            ...
            task_type=(
                "binclass"
                if self.is_binclass
                else "multiclass" if self.is_multiclass else "regression"
            ),
            **model_config,
        ).to(self.args.device)
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        if self.C is None:
            ...
            X_train = np.concatenate(
                (np.array(self.C["train"]), np.array(self.N["train"])), axis=1
            )
            X_val = np.concatenate(
                (np.array(self.C["val"]), np.array(self.N["val"])), axis=1
            )
        ...
        X_train, y_train, X_val, y_val = self.model.preprocess_data(
        ...
        train_dataset = TensorDataset(
            torch.tensor(
                X_train, dtype=torch.float32 if self.args.use_float else torch.float64
            ),
            torch.tensor(
                y_train, dtype=torch.float32 if self.args.use_float else torch.float64
            ),
        )
        self.train_loader = DataLoader(
            train_dataset, batch_size=self.args.batch_size, shuffle=True, drop_last=True
        )

        if X_val is not None and y_val is not None:
            val_dataset = TensorDataset(
                torch.tensor(
                    X_val, dtype=torch.float32 if self.args.use_float else torch.float64
                ),
                torch.tensor(
                    y_val, dtype=torch.float32 if self.args.use_float else torch.float64
                ),
            )
    ...
    def predict(self, data, info, model_name):
        ...
        if self.C is None:
            ...
            X_test = np.concatenate(
                (np.array(self.C_test), np.array(self.N_test)), axis=1
            )
        ...
        self.test_loader = DataLoader(
            test_dataset, batch_size=1024, shuffle=False, drop_last=False
        )
        ...
        test_label__ = (
            test_label.long() if self.is_multiclass else test_label.unsqueeze(1)
        )
        vl = self.criterion(test_logit, test_label__).item()

        if self.is_binclass:
            test_logit = np.stack(
                [-test_logit.cpu().squeeze(), test_logit.cpu().squeeze()], axis=-1
            )
