...
class MitraMethod(Method):
    ...
    def data_format(self, is_train=True, N=None, C=None, y=None):
        if is_train:
            ...
            self.n_classes = (
                self.y_info["n_classes"] if not self.is_regression else None
            )
        else:
    ...
    def construct_model(self, model_config=None):
        from model.models.mitra import Mitra

        if self.is_regression:
            model_path = "./data/models_mitra/reg/"
        else:
            model_path = "./data/models_mitra/cls/"
        self.model = Mitra.from_pretrained(
            path=model_path, device="cpu"  # safe tensor initialization on CPU
        ).to(self.args.device)
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        self.x_support = (
            torch.from_numpy(x_support).float().to(self.args.device)
        )  # [n_support, n_feat]
        self.y_support = (
            torch.from_numpy(y_support).float().to(self.args.device)
        )  # [n_support]
    ...
    def predict(self, data, info, model_name):
        ...
        x_query = (
            torch.from_numpy(Test_X).float().to(self.args.device)
        )  # [n_query, n_feat]
        ...
        with torch.no_grad():
            for start in range(0, n_obs_query, max_samples_query):
                ...
                x_support_batch = self.x_support.unsqueeze(
                    0
                )  # [1, n_obs_support, n_feat]
                ...
                padding_features = torch.zeros(
                    (batch_size, n_feat), dtype=torch.bool, device=self.args.device
                )
                padding_obs_support = torch.zeros(
                    (batch_size, n_obs_support),
                    dtype=torch.bool,
                    device=self.args.device,
                )
                padding_obs_query = torch.zeros(
                    (batch_size, batch_n_query),
                    dtype=torch.bool,
                    device=self.args.device,
                )
        ...
        # test_label, dtype=torch.float32 if self.is_regression else torch.long
        # )
