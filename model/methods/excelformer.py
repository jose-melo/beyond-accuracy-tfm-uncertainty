...
class ExcelFormerMethod(Method):
    ...
    def construct_model(self, model_config=None):
        ...
        self.model = ExcelFormer(
            d_numerical=self.d_in, d_out=self.d_out, **model_config
        ).to(self.args.device)
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        mi_func = (
            mutual_info_regression if self.D.is_regression else mutual_info_classif
        )
        ...
        mi_scores = mi_func(
            self.N["train"].cpu().numpy(), self.y["train"].cpu().numpy()
        )
        mi_ranks = np.argsort(-mi_scores)
        self.sorted_mi_scores = (
        ...
        for i, (X, y) in enumerate(self.train_loader, 1):
            ...
            if self.mix_type == "none":
                ...
                if self.is_regression:
                    ...
                    loss = lambdas * self.criterion(
                        preds, y
                    ) + lambdas2 * self.criterion(preds, y[shuffled_ids])
                    loss = loss.mean()
            tl.add(loss.item())
