...
class GrowNetMethod(Method):
    ...
    def construct_model(self, model_config=None):
        ...
        self.model = DynamicNet(
            categories=self.categories, **self.ensemble_model_config
        )
        self.model.to_cuda()
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        if self.C:
            self.sub_model_config["feat_d"] = (
                self.d_in
                + self.C["train"].shape[1] * self.ensemble_model_config["d_embedding"]
            )
        else:
        ...
        for s in range(self.args.max_epoch):
            m = MLP_2HL.get_model(s, argparse.Namespace(**self.sub_model_config)).to(
                self.args.device
            )
            ...
            for epoch in range(training_config["epochs_per_stage"]):
                for i, (X, y) in enumerate(self.train_loader, 1):
                    ...
                    if self.is_regression:
                        ...
                        out = (
                            torch.as_tensor(out, dtype=torch.float64).cuda().view(-1, 1)
                        )
                        ...
                        loss = self.loss_f1(
                            self.model.boost_rate * out, grad_direction
                        )  # T
                        loss = loss * h
                        loss = loss.mean()
                    m.zero_grad()
            ...
            if s > 0:
                ...
                for e in range(training_config["correct_epoch"]):
                    for i, (X, y) in enumerate(self.train_loader, 1):
                        ...
                        self.optimizer.step()
                    print(
                        f"Corrected epoch {e} done with loss {loss.item()} for stage {s} with lr {self.optimizer.param_groups[0]['lr']}"
                    )
            self.validate(s)
    ...
    def predict(self, data, info, model_name):
        ...
        self.model = DynamicNet.from_file(
            ...
            lambda stage: MLP_2HL.get_model(
                stage, argparse.Namespace(**self.sub_model_config)
            ),
        )
        self.model.to_eval()
        self.model.to_cuda()
