...
class ProtoGateMethod(Method):
    ...
    def construct_model(self, model_config=None):
        ...
        self.model = GatingNet(input_dim=self.d_in, **model_config).to(self.args.device)
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        self.feature_selection = (
            self.args.config["model"]["feature_selection"]
            if "feature_selection" in self.args.config["training"].items()
            else True
        )
        ...
        self.proto_layer = KNNNet(
            k=self.pred_k, tau=self.args.config["training"]["sorting_tau"]
        )
    ...
    def predict(self, data, info, model_name):
        ...
        with torch.no_grad():
            for i, (X, y) in tqdm(enumerate(self.test_loader)):
                ...
                x_neighbour_selected, _, _ = self.model(
                    torch.tensor(self.X_train).to(x_selected.device)
                )
                y_neighbour = self.y_neighbour.to(x_selected.device)
                y_neighbour = F.one_hot(y_neighbour, num_classes=self.d_out)

                pred, neighbour_list = proto_predict(
                    x_selected, x_neighbour_selected, y_neighbour, self.pred_k
                )
    ...
    def train_epoch(self, epoch):
        ...
        for i, (X, y) in enumerate(self.train_loader, 1):
            ...
            y_pred, neighbour_list = proto_predict(
                x_selected, x_neighbour_selected, y_neighbour, self.pred_k
            )
            loss = self.compute_pred_loss(
                x_selected, x_neighbour_selected, y_true, y_neighbour
            )["total"]
    ...
    def compute_pred_loss(self, x_query, x_cand, y_query, y_neighbor):
        ...
        if self.feature_selection:
            ...
            losses["l1_norm"] = self.l1_coef * torch.norm(
                self.model.embed.fn0.weight, p=1
            )

        losses["pred_loss"] = self.pred_k + pred_loss(
            self.proto_layer, x_query, x_cand, y_query, y_neighbor
        )
...
def pred_loss(
    proto_layer, query, neighbors, query_label, neighbor_labels, method="deterministic"
):
    ...
    if method == "deterministic":
        ...
        correct = (query_label.unsqueeze(1) * neighbor_labels.unsqueeze(0)).sum(
            -1
        )  # (B, N)
