...
class ModernNCAMethod(Method):
        ...
        with torch.no_grad():
            for i, (X, y) in tqdm(enumerate(self.test_loader)):
                ...
                if self.args.use_float:
                    ...
                    candidate_x_num = (
                        candidate_x_num.float() if candidate_x_num is not None else None
                    )
                    candidate_x_cat = (
                        candidate_x_cat.float() if candidate_x_cat is not None else None
                    )
                    if self.is_regression:
                ...
                if X_cat is None and X_num is not None:
                    ...
                    x, candidate_x = torch.cat([X_num, X_cat], dim=1), torch.cat(
                        [candidate_x_num, candidate_x_cat], dim=1
                    )
    ...
    def train_epoch(self, epoch):
        ...
        tl = Averager()
        i = 0
        for batch_idx in make_random_batches(
            self.train_size, self.args.batch_size, self.args.device
        ):
            ...
            candidate_indices = candidate_indices[
                ~torch.isin(candidate_indices, batch_idx)
            ]

            candidate_x_num = (
                self.N["train"][candidate_indices] if self.N is not None else None
            )
            candidate_x_cat = (
                self.C["train"][candidate_indices] if self.C is not None else None
            )
