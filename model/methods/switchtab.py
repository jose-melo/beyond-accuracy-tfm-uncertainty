...
class SwitchTabMethod(Method):
    ...
    def data_format(self, is_train=True, N=None, C=None, y=None):
        if is_train:
            ...
            if self.N["train"].shape[1] % 2 != 0:
                self.N["train"] = np.hstack(
                    (self.N["train"], np.zeros((self.N["train"].shape[0], 1)))
                )
                self.N["val"] = np.hstack(
                    (self.N["val"], np.zeros((self.N["val"].shape[0], 1)))
                )
            ...
            if N_test["test"].shape[1] % 2 != 0:
                N_test["test"] = np.hstack(
                    (N_test["test"], np.zeros((N_test["test"].shape[0], 1)))
                )
    ...
    def train_epoch(self, epoch):
        ...
        for i, ((X1, y1), (X2, y2)) in enumerate(
            zip(self.train_loader, self.train_loader), 1
        ):
            ...
            X1_recon, X2_recon, X1_switched, X2_switched, X1_pred, X2_pred, alpha = (
                self.model(X1_num, X2_num)
            )

            recon_loss = (
                self.recon_criterion(X1_recon, X1_num)
                + self.recon_criterion(X1_switched, X1_num)
                + self.recon_criterion(X2_recon, X2_num)
                + self.recon_criterion(X2_switched, X2_num)
            )
