...
from model.lib.ptarl.utils import fit_Ptarl, test, generate_topic
...
class PTARLMethod(Method):
    ...
    def fit(self, data, info, train=True, config=None):
        ...
        best_model, _ = fit_Ptarl(
            ...
            self.args.config,
            self.args.config["model"]["regularize"],
            self.is_regression,
            self.args.config["general"]["ot_weight"],
            self.args.config["general"]["diversity_weight"],
            self.args.config["general"]["r_weight"],
            self.args.config["general"]["diversity"],
            self.args.seed,
            self.args.save_path,
        )
        cluster_centers_ = generate_topic(
            best_model, self.train_loader, self.args.config["model"]["n_clusters"]
        )
        self.cluster_centers_ = cluster_centers_
        np.save(
            osp.join(
                self.args.save_path,
                "cluster-centers-{}.npy".format(str(self.args.seed)),
            ),
            cluster_centers_,
        )
        self.model_type1 = self.args.model_type + "_ot"
        self.construct_model(self.model_config)
        best_model, best_loss = fit_Ptarl(
            self.args,
            self.model,
            self.train_loader,
            self.val_loader,
            self.criterion,
            self.args.model_type + "_ot",
            self.args.config,
            self.args.config["model"]["regularize"],
            self.is_regression,
            self.args.config["general"]["ot_weight"],
