...
class GRANDE(nn.Module):
    ...
    def forward(self, inputs):
        ...
        split_index_array = split_index_array - (
            split_index_array
        ...
        node_result_extended = node_result_corrected[
        ...
        estimator_weights_leaf_softmax = torch.nn.functional.softmax(
            estimator_weights_leaf, dim=-1
        )

        if self.dropout > 0 and training:
            estimator_weights_leaf_softmax = nn.functional.dropout(
                estimator_weights_leaf_softmax, p=self.dropout
            )

        if self.task_type == "regression":
            layer_output = torch.einsum("el,bel->be", self.leaf_classes_array, p)
            layer_output = torch.einsum(
                "be,be->be", estimator_weights_leaf_softmax, layer_output
            )
        elif self.task_type == "binclass":
            if self.from_logits:
                layer_output = torch.einsum("el,bel->be", self.leaf_classes_array, p)
            else:
                layer_output = torch.sigmoid(
                    torch.einsum("el,bel->be", self.leaf_classes_array, p)
                )
            layer_output = torch.einsum(
                "be,be->be", estimator_weights_leaf_softmax, layer_output
            )
            ...
            if self.from_logits:
                ...
                layer_output = torch.einsum(
                    "be,bec->bec", estimator_weights_leaf_softmax, layer_output
                )
            else:
                layer_output = torch.nn.functional.softmax(
                    torch.einsum("elc,bel->bec", self.leaf_classes_array, p), dim=-1
                )
                layer_output = torch.einsum(
                    "be,bec->bec", estimator_weights_leaf_softmax, layer_output
                )
    ...
    def build_model(self):
        ...
        if self.data_subset_fraction < 1.0:
            ...
            if self.bootstrap:
                self.data_select = torch.randint(
                    0, self.batch_size, (self.n_estimators, self.subset_size)
                )
            else:
                indices = [
                    np.random.choice(
                        self.batch_size, size=self.subset_size, replace=False
                    )
            ...
            self.counts = torch.tensor(
                np.unique(self.data_select.cpu().numpy(), return_counts=True)[1],
                dtype=torch.float64,
            )

        self.features_by_estimator = torch.tensor(
            [
                np.random.choice(
        ...
        self.path_identifier_list = (
            torch.tensor(self.path_identifier_list).view(-1, self.depth).double()
        )
        self.internal_node_index_list = (
            torch.tensor(self.internal_node_index_list).view(-1, self.depth).long()
        )
    ...
    def entmax_threshold_and_support(self, inputs, axis=-1):
        ...
        rho = torch.arange(
            1, num_outcomes + 1, device=inputs.device, dtype=inputs.dtype
        )
