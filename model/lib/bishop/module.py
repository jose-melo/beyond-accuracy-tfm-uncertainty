    ...
    def __init__(
        ...
        for i, (dim_in, dim_out) in enumerate(dim_pairs):
            ...
            if bn and (not is_last or bn_final):
                layers.append(torch.nn.BatchNorm1d(dim_in))
        ...
        self.shortcut = (
            torch.nn.Linear(mlp_configs[0], mlp_configs[-1]) if skip_connect else None
        )
...
class BAModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pos_enc_encoder = torch.nn.Parameter(
            torch.randn(1, emb_dim, self.pad_feat_dim // patch_dim, d_model)
        )
        ...
        if d_layer > 0:
            if d_layer > (e_layer + 1):
                d_layer = e_layer + 1
            self.pos_enc_decoder = torch.nn.Parameter(
                torch.randn(1, emb_dim, n_pool_dec, d_model)
            )
            ...
            self.decoder = torch.nn.Linear(
                self.n_pool_records[-1] * d_model, self.pad_out_dim
            )
        else:
            self.decoder = torch.nn.Linear(
                self.n_pool_total * d_model, self.pad_out_dim
            )
    ...
    def forward(self, x):
        ...
        if self.d_layer > 0:
            dec_in = repeat(
                self.pos_enc_decoder,
                "b emb_dim n_pool_dec d_model -> (repeat b) emb_dim n_pool_dec d_model",
                repeat=batch,
            )
        ...
        enc_final = rearrange(
            enc_final, "b emb_dim n_pools d_model -> b emb_dim (n_pools d_model)"
        )
