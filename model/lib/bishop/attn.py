...
class GSH(torch.nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        self.scale = self.scale or 1.0 / math.sqrt(E)
...
class BAModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.feat_attn = GSHLayer(
            d_model, n_heads, dropout=dropout, actv=actv, hopfield=hopfield
        )
        self.emb_pooling = GSHLayer(
            d_model, n_heads, dropout=dropout, actv=actv, hopfield=hopfield
        )
        self.emb_attn = GSHLayer(
            d_model, n_heads, dropout=dropout, actv=actv, hopfield=hopfield
        )
    ...
    def forward(self, x):
        ...
        feat_enc = self.feat_attn(feat_in, feat_in, feat_in)
        ...
        emb_send = rearrange(
            emb_in, "(b emb_dim) n_pool d_model -> (b n_pool) emb_dim d_model", b=batch
        )
        pooling_send = repeat(
            self.pooling,
            "n_pool factor d_model -> (repeat n_pool) factor d_model",
            repeat=batch,
        )
        ...
        final_out = rearrange(
            emb_out, "(b n_pool) emb_dim d_model -> b emb_dim n_pool d_model", b=batch
        )
        return final_out
