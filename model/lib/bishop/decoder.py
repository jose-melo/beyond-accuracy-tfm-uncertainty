...
class DecoderLayer(torch.nn.Module):
    ...
    def forward(self, x, enc_x):
        ...
        enc_x = rearrange(
            enc_x, "b emb_dim n_pool d_model -> (b emb_dim) n_pool d_model"
        )
        tmp = self.GSHLayer(
            x,
            enc_x,
        ...
        dec_out = rearrange(
            dec_out, "(b emb_dim) n_out d_model -> b emb_dim n_out d_model", b=batch
        )
        layer_predict = self.linear(dec_out)
        layer_predict = rearrange(
            layer_predict, "b emb_dim n_out seg_len -> b (emb_dim n_out) seg_len"
        )
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, x, enc):
        ...
        final_predict = rearrange(
            final_predict,
            "b (emb_dim n_patch) patch_dim -> b (n_patch patch_dim) emb_dim",
            emb_dim=emb_dim,
        )
