...
class TabModel(BaseEstimator):
    ...
    def save_model(self, path):
        ...
        # Save models params
        with open(
            Path(path).joinpath(f"model_params-{self.seed}.json"), "w", encoding="utf8"
        ) as f:
        ...
        torch.save(
            ...
            Path(path).joinpath(f"epoch-last-{self.seed}.pth"),
        )
    ...
    def load_model(self, filepath, seed):
        ...
        try:
            ...
            saved_state_dict = torch.load(
                osp.join(filepath, f"epoch-last-{seed}.pth"), map_location=self.device
            )
