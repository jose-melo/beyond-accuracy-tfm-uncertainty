...
class PrimaryCapsuleGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        return out.transpose(
            -1, -2
        )  # 最后的输出维度为（batch_size，capsule_dim+1，num_feature），将x和capsule之后的结果cat起来
...
class InferCapsule(nn.Module):
    def __init__(
        ...
        self.thread = nn.Parameter(
            torch.rand(1, in_capsule_num, out_capsule_num), requires_grad=True
        )
...
class CapsuleEncoder(nn.Module):
    def __init__(
        ...
        self.digit_capsules = InferCapsule(
            in_capsule_num=primary_capsule_dim + 1,
            out_capsule_num=out_capsule_num,
            in_capsule_size=digit_input_dim,
            out_capsule_size=digit_capsule_dim,
            n_leaves=n_leaves,
        )
...
class ReconstructCapsNet(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        y = y.repeat(1, (hidden.shape[1] // self.num_class)).view(
            y.shape[0], -1, self.num_class
        )
        hidden = hidden.view(
