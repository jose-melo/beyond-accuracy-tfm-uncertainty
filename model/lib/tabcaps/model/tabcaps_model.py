...
class TabCapsModel(BaseEstimator):
    ...
    optimizer_params: Dict = field(
        default_factory=lambda: dict(lr=2e-2, weight_decay=1e-5, nus=(0.8, 1.0))
    )
    scheduler_fn: Any = torch.optim.lr_scheduler.StepLR
    scheduler_params: Dict = field(
        default_factory=lambda: dict(gamma=0.95, step_size=20)
    )
    input_dim: int = None
    output_dim: int = None
    ...
    def fit(
        ...
        self.log = (
            Train_Log(logname, resume_dir, cfg) if (logname or resume_dir) else None
        )
        ...
        if resume_dir:
            start_epoch, self.network, self._optimizer, best_value, best_epoch = (
                self.log.load_checkpoint(self._optimizer)
            )
    ...
    def predict(self, X, y, decode=False):
        ...
        if decode == True:
            ...
            reconstruct_data = np.concatenate(
                [reconstruct_data, y_list[:, None]], axis=1
            )
    ...
    def _train_epoch(self, train_loader):
        ...
        epoch_logs = {
            "lr": self._optimizer.param_groups[-1]["lr"],
            "loss": np.mean(loss),
        }
    ...
    def _set_network(self):
        ...
        self.params = {
            "out_capsule_num": self.output_dim * self.sub_class,
        ...
        self.network = (
            ReconstructCapsNet(self.input_dim, self.output_dim, **self.params)
            if self.decode
            else CapsuleClassifier(self.input_dim, self.output_dim, **self.params)
        )
    ...
    def _construct_loaders(self, X_train, y_train, eval_set):
        ...
        train_dataloader, valid_dataloaders = create_dataloaders(
            X_train, y_train_mapped, eval_set, self.batch_size
        )
