...
def get_dataset(dataset_name, dataset_path):
    ...
    N_trainval = (
        ...
        if N is None
            ...
            {key: N[key] for key in ["train", "val"]}
    ...
    N_test = (
        None
        if N is None
        else {key: N[key] for key in ["test"]} if "test" in N else None
    )
    ...
    C_trainval = (
        ...
        if C is None
            ...
            {key: C[key] for key in ["train", "val"]}
    ...
    C_test = (
        None
        if C is None
        else {key: C[key] for key in ["test"]} if "test" in C else None
    )
...
def data_loader_process(
    is_regression, X, Y, y_info, device, batch_size, is_train, is_float=False
):
    """
