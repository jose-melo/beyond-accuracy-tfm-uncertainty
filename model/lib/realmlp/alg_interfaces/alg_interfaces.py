...
class AlgInterface:
    ...
    def fit_and_eval(
        ...
        return self.eval(
            ds=ds, idxs_list=idxs_list, metrics=metrics, return_preds=return_preds
        )
    ...
    def eval(
        ...
        for split_idx, idxs in enumerate(idxs_list):
            ...
            if return_preds:
                results["y_preds"] = y_preds.cpu().numpy().tolist()
            ...
            if idxs.test_idxs is not None:
                ...
                results["metrics", "test"] = metrics.compute_metrics_dict(
                    y_preds=[
                        y_preds[i, idxs.test_idxs] for i in range(y_preds.shape[0])
                    ],
                    y=y[idxs.test_idxs],
                    use_ens=True,
                )
            ...
            for i in range(idxs.n_trainval_splits):
                train_dict = metrics.compute_metrics_dict(
                    [y_preds[i, idxs.train_idxs[i]]],
                    y[idxs.train_idxs[i]],
                    use_ens=False,
                )
                ...
                if idxs.val_idxs is not None and idxs.val_idxs.shape[-1] > 0:
                    val_dict = metrics.compute_metrics_dict(
                        [y_preds[i, idxs.val_idxs[i]]],
                        y[idxs.val_idxs[i]],
                        use_ens=False,
                    )
