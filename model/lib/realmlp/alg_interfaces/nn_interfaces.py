...
from model.lib.realmlp.alg_interfaces.base import (
    SplitIdxs,
    InterfaceResources,
    RequiredResources,
)
...
class NNAlgInterface(AlgInterface):
    ...
    def get_refit_interface(
        self, n_refit: int, fit_params: Optional[List[Dict]] = None
    ) -> "AlgInterface":
        return NNAlgInterface(
            fit_params if fit_params is not None else self.fit_params, **self.config
        )
    ...
    def fit(
        ...
        logger: Logger,
        tmp_folders: List[Optional[Path]],
        name: str,
    ):
    ...
    def predict(self, ds: DictDataset) -> torch.Tensor:
        ...
        y_pred = self.trainer.predict(
            model=self.model, dataloaders=self.model.get_predict_dataloader(ds_x)
        )
        y_pred = cat_if_necessary(y_pred, dim=-2).to(
            "cpu"
    ...
    def get_required_resources(
        ...
        ds_size_gb = (
            ...
            * sum(
                [
                    ti.get_n_features() * (8 if ti.is_cat() else 4)
                    for ti in static_tensor_infos.values()
                ]
            )
            / (1024**3)
        )
        ...
        param_memory = (
            5 * n_params * 8
        )  # 5 because of model, model copy, grads, adam mom, adam sq_mom
        ...
        gpu_usage = min(
            1.0, n_parallel / 100
        )  # rather underestimate it and use up all the ram on the gpu
        ...
        if self.config.get("use_gpu", True):
            ...
            return RequiredResources(
                time_s=time_approx, n_threads=1.0, cpu_ram_gb=cpu_ram_gb + gpu_ram_gb
            )

    def get_model_ram_gb(
        self,
        ds: DictDataset,
        n_cv: int,
        n_refit: int,
    ...
    def get_importances(self) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            importances /= importances.norm(dim=-1, keepdim=True) / np.sqrt(
                importances.shape[-1]
            )
            return importances
...
class RealMLPParamSampler:
    ...
    def sample_params(self, seed: int) -> Dict[str, Any]:
        ...
        params = {
            "num_emb_type": rng.choice(["none", "pbld", "pl", "plr"]),
            ...
            "hidden_sizes": hidden_size_options[
                rng.choice([0, 1, 2], p=[0.6, 0.2, 0.2])
            ],
        }
