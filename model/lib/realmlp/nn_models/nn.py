...
from model.lib.realmlp.nn_models.base import (
    Fitter,
    Variable,
    WeightLayer,
    BiasLayer,
...
class WeightFitter(Fitter):
    def __init__(self, out_features, **config):
        ...
        if self.use_ntk_param:
            raise ValueError(
                f'use_ntk_param is discontinued, use weight_param="ntk" instead'
            )
        if self.use_ntk_param_v2:
            raise ValueError(
                f'use_ntk_param_v2 is discontinued, use weight_param="ntk-v2" instead'
            )
        if self.use_ntk_param_v3:
            raise ValueError(
                f'use_ntk_param_v3 is discontinued, use weight_param="ntk-v3" instead'
            )
        ...
        super().__init__(
            needs_tensors=self.weight_init_mode in ["std"]
        )  # todo: adjust for some weight init modes
    ...
    def forward_tensor_infos(self, tensor_infos):
        return utils.update_dict(
            tensor_infos, {"x_cont": TensorInfo(feat_shape=[self.out_features])}
        )
    ...
    def _fit(self, ds: DictDataset):
        ...
        if self.weight_param == "xavier":
            ...
            init_factor = self.weight_init_gain * np.sqrt(
                2.0 / (in_features + self.out_features)
            )
            ...
            init_factor = (
                self.weight_init_gain * self.weight_gain * np.sqrt(1.0 / in_features)
            )
            lr_factor = (
                self.weight_lr_factor
                * self.weight_gain
                / in_features
            ...
            if self.layer_position == "first":
                ...
                raise ValueError(
                    f"Unknown layer_position for mup-adam: {self.layer_position}"
                )
            ...
            if self.layer_position == "first":
                ...
                raise ValueError(
                    f"Unknown layer_position for mup-adam-custom: {self.layer_position}"
                )
            ...
            if self.layer_position == "first":
                ...
                raise ValueError(
                    f"Unknown layer_position for mup-adam-custom-2: {self.layer_position}"
                )
            ...
            if self.layer_position == "first":
                ...
                raise ValueError(
                    f"Unknown layer_position for mup-sgd-custom: {self.layer_position}"
                )
        ...
        if self.weight_init_mode == "normal":
            ...
            weight = np.sqrt(3) * (
                2 * torch.rand(in_features, self.out_features, device=ds.device) - 1
            )
            ...
            weight = torch.randn(in_features, self.out_features, device=ds.device)
            x = ds.tensors["x_cont"]
            weight = weight / x.matmul(weight_gain * init_factor * weight).std(
                dim=-2, correction=0, keepdim=True
            )
            ...
            weight = (
                weight
                / x.matmul(weight_gain * init_factor * weight)
                .square()
        ...
        if self.use_norm_weight:
            factor = (
                np.sqrt(self.out_features / in_features)
                if self.norm_weight_transpose
                else 1.0
            )
            return NormWeightLayer(
                Variable(
                    init_factor * weight,
                    trainable=True,
                    hyper_factors={
                ...
                factor=weight_gain * factor,
                fitter=self,
                transpose=self.norm_weight_transpose,
            )
        else:
            return WeightLayer(
                Variable(
...
class BiasFitter(Fitter):
    def __init__(self, **config):
        ...
        super().__init__(
            needs_tensors=self.bias_init_mode
            in [
                "he+5",
                "mean",
                "neg-uniform-dynamic",
                "neg-uniform-dynamic-2",
                "normal-dynamic",
            ]
        )
    ...
    def heplus_bias(self, x, n_simplex):
        idxs = torch.randint(
            0, x.shape[0], size=(x.shape[1], n_simplex), device=x.device
        )
        simplex_weights = torch.distributions.Exponential(1.0).sample(
            (x.shape[1], n_simplex)
        )
        simplex_weights = simplex_weights.to(x.device)
        simplex_weights /= simplex_weights.sum(dim=1)[:, None]
        out_selected = torch.stack(
            [
                x[idxs[:, i], torch.arange(x.shape[1], device=x.device)]
    ...
    def _fit(self, ds: DictDataset):
        ...
        if self.bias_init_mode == "zeros" or self.bias_init_mode == "zero":
            ...
            bias = np.sqrt(1.0 / self.in_features) * (
                2 * torch.rand(n_features, device=ds.device) - 1
            )
        else:
        ...
        return BiasLayer(
            Variable(
                self.bias_init_gain * bias[None, :] / bias_gain,
                trainable=True,
                hyper_factors={
                    "lr": lr_factor,
                    "wd": wd_factor,
...
class DropoutLayer(Layer):
    def __init__(self):
        super().__init__()
        self.hyper_getter = self.context.hp_manager.register_hyper(
            "p_drop", self.context.scope
        )
...
class NoiseLayer(Layer):
    def __init__(self):
        super().__init__()
        self.sigma_getter = self.context.hp_manager.register_hyper(
            "layer_noise_sigma", self.context.scope
        )
...
class ClampLayer(Layer):
    ...
    def _stack(self, layers):
        return ClampLayer(
            Variable.stack([l.low for l in layers]),
...
class ClampOutputFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        y = (
            TrainContext.get_global_context()
            .hp_manager.get_more_info_dict()["trainval_ds"]
            .tensors["y"]
        )
        return ClampLayer(
            low=Variable(y.min(dim=-2, keepdim=True)[0], trainable=False),
...
class NormalizeOutputLayer(Layer):
    ...
    def _stack(self, layers):
        return NormalizeOutputLayer(
            mean=Variable.stack([l.mean for l in layers]),
...
class NormalizeOutputFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return NormalizeOutputLayer(
            mean=Variable(y.mean(dim=-2, keepdim=True), trainable=False),
...
class NormWeightLayer(Layer):
    def __init__(
        self, weight: Variable, factor: float, fitter: Fitter, transpose=False
    ):
    ...
    def forward_cont(self, x):
        return x.matmul(
            self.factor
            * self.weight
            / self.weight.norm(dim=-1 if self.transpose else -2, keepdim=True)
        )
...
class FixedScaleFactory(Fitter, FitterFactory):
    def __init__(self, scale: torch.Tensor):
        super().__init__(
            needs_tensors=False, is_individual=True, modified_tensors=["x_cont"]
        )
        self.scale = scale
...
class FeatureImportanceFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        scale = TrainContext.get_global_context().hp_manager.get_more_info_dict()[
            "feature_importances"
        ][None, :]
...
class FixedWeightFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        weight = TrainContext.get_global_context().hp_manager.get_more_info_dict()[
            "fixed_weights"
        ]
...
class RFFeatureImportanceFactory(Fitter, FitterFactory):
    def __init__(self):
        super().__init__(
            needs_tensors=True, is_individual=True, modified_tensors=["x_cont"]
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        scale = torch.as_tensor(
            model.feature_importances_, dtype=torch.float32, device=ds.device
        )
...
class PLREmbeddingsFactory(Fitter, FitterFactory):
    ...
    def __init__(
        ...
        plr_lr_factor_1: float = 1.0,
        plr_lr_factor_2: float = 1.0,
        plr_wd_factor: float = 1.0,
        plr_act_name: str = "relu",
        plr_use_densenet: bool = False,
        plr_use_cos_bias: bool = False,
        **config,
    ):
        super().__init__(
            needs_tensors=False, is_individual=True, modified_tensors=["x_cont"]
    ...
    def get_n_params(self, tensor_infos: Dict[str, TensorInfo]) -> int:
        ...
        if self.plr_use_cos_bias:
            ...
            return n_cont * (
                self.plr_hidden_1 // 2 + (self.plr_hidden_1 + 1) * hidden_2
            )
    ...
    def forward_tensor_infos(
        ...
        return utils.update_dict(
            tensor_infos,
            {
                "x_cont": TensorInfo(
                    feat_shape=[
                        tensor_infos["x_cont"].get_n_features() * self.plr_hidden_2
                    ]
                )
            },
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        n_cont = ds.tensor_infos[
            "x_cont"
        ].get_n_features()  # assuming that the shape is rank 1
        hyper_factors_1 = {
            "lr": self.plr_lr_factor * self.plr_lr_factor_1,
            "wd": self.plr_wd_factor,
        }
        hyper_factors_2 = {
            "lr": self.plr_lr_factor * self.plr_lr_factor_2,
            "wd": self.plr_wd_factor,
        }
        ...
        if self.plr_use_cos_bias:
            with sub_scope_context("weight_1"):
                weight_1 = Variable(
                    self.plr_sigma
                    * torch.randn(n_cont, 1, self.plr_hidden_1, device=ds.device),
            ...
            with sub_scope_context("bias_1"):
                ...
                bias_1 = Variable(
                    ...
                    * (
                        -1
                        + 2 * torch.rand(n_cont, 1, self.plr_hidden_1, device=ds.device)
                    ),
                    hyper_factors=hyper_factors_1,
                )
        else:
            ...
            with sub_scope_context("weight_1"):
                weight_1 = Variable(
                    self.plr_sigma
                    * torch.randn(n_cont, 1, self.plr_hidden_1 // 2, device=ds.device),
                    hyper_factors=hyper_factors_1,
                )
        ...
        with sub_scope_context("weight_2"):
            weight_2 = Variable(
                (
                    -1
                    + 2
                    * torch.rand(n_cont, self.plr_hidden_1, hidden_2, device=ds.device)
                )
                / np.sqrt(in_features),
        ...
        with sub_scope_context("bias_2"):
            bias_2 = Variable(
                (-1 + 2 * torch.rand(n_cont, 1, hidden_2, device=ds.device))
                / np.sqrt(in_features),
                hyper_factors=hyper_factors_2,
            )

        if self.plr_use_cos_bias:
            return PLREmbeddingsLayerCosBias(
                fitter=self,
                weight_1=weight_1,
                weight_2=weight_2,
                bias_1=bias_1,
                bias_2=bias_2,
                plr_act_name=self.plr_act_name,
                plr_use_densenet=self.plr_use_densenet,
            )
        else:
            return PLREmbeddingsLayer(
                fitter=self,
                weight_1=weight_1,
                weight_2=weight_2,
                bias_2=bias_2,
...
class PeriodicEmbeddingsFactory(Fitter, FitterFactory):
    ...
    def __init__(
        self,
        periodic_emb_sigma: float = 1.0,
        periodic_emb_dim: int = 8,
        periodic_emb_lr_factor: float = 1.0,
        periodic_emb_wd_factor: float = 1.0,
        periodic_emb_only_cos: bool = False,
        periodic_emb_densenet: bool = False,
        **config,
    ):
        super().__init__(
            needs_tensors=False, is_individual=True, modified_tensors=["x_cont"]
    ...
    def forward_tensor_infos(
        ...
        return utils.update_dict(
            tensor_infos,
            {
                "x_cont": TensorInfo(
                    feat_shape=[
                        tensor_infos["x_cont"].get_n_features() * self.periodic_emb_dim
                    ]
                )
            },
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        hyper_factors = {
            "lr": self.periodic_emb_lr_factor,
            "wd": self.periodic_emb_wd_factor,
        }
        ...
        if self.periodic_emb_only_cos:
            ...
            if param_dim % 2 == 1:
                raise ValueError(
                    f"Wrong parity for periodic_emb_dim, got {self.periodic_emb_dim=}"
                )
            param_dim //= 2

            with sub_scope_context("weight"):
                weight = Variable(
                    self.periodic_emb_sigma
                    * torch.randn(n_cont, 1, param_dim, device=ds.device),
                    hyper_factors=hyper_factors,
                )

            return PeriodicEmbeddingsLayerSinCos(
                self, weight, periodic_emb_densenet=self.periodic_emb_densenet
            )
...
class ToSoftLabelLayer(Layer):
    ...
    def forward_tensors(self, tensors):
        if "y" not in tensors:
            ...
            y = tensors["y"]
            y_cs = self.y_tensor_info.get_cat_sizes().cpu().numpy()
            new_y_cols = []
            for i, cs in enumerate(y_cs):
                if cs == 0:
                    # already continuous
                    new_y_cols.append(
                        y[[slice(None)] * (y.dim() - 1) + [slice(i, i + 1)]]
                    )
                else:
                    # make continuous
                    ...
                    new_y_cols.append(
                        F.one_hot(
                            y[[slice(None)] * (y.dim() - 1) + [i]], num_classes=cs
                        ).float()
                    )
...
class ToSoftLabelFitter(Fitter):
    def __init__(self):
        super().__init__(
            needs_tensors=False, is_individual=False, modified_tensors=["y"]
        )
    ...
    def forward_tensor_infos(self, tensor_infos):
        ...
        new_y_shape = sum(
            [max(1, cs) for cs in tensor_infos["y"].get_cat_sizes().cpu().numpy()]
        )
        return utils.update_dict(
            tensor_infos, {"y": TensorInfo(feat_shape=[new_y_shape])}
        )
...
class LabelSmoothingFactory(FitterFactory):
    ...
    def _create(self, tensor_infos) -> Fitter:
        if tensor_infos["y"].get_cat_sizes()[0].item() > 0:
            ...
            return SequentialFitter(
                [ToSoftLabelFitter(), LabelSmoothingFitter(**self.config)]
            )
...
class StochasticLabelNoiseLayer(Layer):
    ...
    def forward_tensors(self, tensors):
        ...
        return utils.join_dicts(
            tensors, {"y": y + self.sigma_getter() * torch.randn_like(y)}
        )
...
class StochasticLabelNoiseFitter(Fitter):
    def __init__(self):
        super().__init__(
            needs_tensors=False, is_individual=False, modified_tensors=["x_cont"]
        )
...
class StochasticGateLayer(Layer):
    def __init__(self, mu: Variable):
        super().__init__()
        self.sigma_getter = self.context.hp_manager.register_hyper(
            "sg_sigma", self.context.scope
        )
        self.lambda_getter = self.context.hp_manager.register_hyper(
            "sg_lambda", self.context.scope
        )
        self.mu = mu
...
class StochasticGateFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return StochasticGateLayer(
            Variable(
                0.5 * torch.ones(1, n_cont, device=ds.device), hyper_factors={"wd": 0.0}
            )
        )
...
class AntisymmetricInitializationFactory(FitterFactory):
    ...
    def _create(self, tensor_infos) -> Fitter:
        ...
        return SequentialFitter(
            [common, AntisymmetricInitializationFitter(individual, **self.config)]
        )
...
class AntisymmetricInitializationFitter(Fitter):
    ...
    def __init__(self, fitter: Fitter, **config):
        super().__init__(
            needs_tensors=fitter.needs_tensors,
            is_individual=fitter.is_individual,
            scope_names=fitter.scope_names,
            modified_tensors=fitter.modified_tensors,
        )
        self.fitter = fitter
    ...
    def get_n_forward(self, tensor_infos: Dict[str, TensorInfo]):
        return 2 * self.fitter.get_n_forward(
            tensor_infos
        )  # maybe not entirely accurate but almost
...
class SubtractionLayer(Layer):
    ...
    def forward_tensor_infos(self, tensor_infos):
        return utils.join_dicts(
            self.layer1.forward_tensor_infos(tensor_infos),
    ...
    def _stack(self, layers):
        return SubtractionLayer(
            layers[0].layer1.stack([l.layer1 for l in layers]),
