...
class ReplaceMissingContLayer(Layer):
    ...
    def _stack(self, layers: List["ReplaceMissingContLayer"]):
        return ReplaceMissingContLayer(
            Variable.stack([layer.means for layer in layers])
        )
...
class MeanCenterFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return BiasLayer(
            Variable(
                -ds.tensors["x_cont"].mean(dim=-2, keepdim=True),
...
class MedianCenterFactory(Fitter, FitterFactory):
    def __init__(self, median_center_trainable=False, **config):
        super().__init__(
            is_individual=median_center_trainable, modified_tensors=["x_cont"]
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return BiasLayer(
            Variable(
                -torch_np_quantile(ds.tensors["x_cont"], 0.5, dim=-2, keepdim=True),
                trainable=self.trainable,
            )
        )
...
class L2NormalizeFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        scale = (
            1.0
            / (ds.tensors["x_cont"] ** 2 + self.eps).mean(dim=-2, keepdim=True).sqrt()
        )
...
class RobustScaleFactory(Fitter, FitterFactory):
    def __init__(self, robust_scale_trainable=False, robust_scale_eps=1e-30, **config):
        super().__init__(
            is_individual=robust_scale_trainable, modified_tensors=["x_cont"]
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        if ds.tensor_infos["x_cont"].is_empty():
            return IdentityLayer()
        x_cont = ds.tensors["x_cont"]
        quant_diff = torch_np_quantile(x_cont, 0.75, dim=-2) - torch_np_quantile(
            x_cont, 0.25, dim=-2
        )
...
class RobustScaleV2Factory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        possible_factors = (
            2.0
            * opposite_quantile_dists[:quarter_idx, None]
...
class GlobalScaleNormalizeFactory(Fitter, FitterFactory):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return ScaleLayer(
            Variable(scale * torch.ones(1, 1, device=x_cont.device), trainable=False)
        )
...
class ThermometerCodingLayer(Layer):
    ...
    def _stack(self, layers):
        return ThermometerCodingLayer(
            Variable.stack([l.centers for l in layers]),
            layers[0].scale,
            layers[0].fitter,
        )
...
class ThermometerCodingFactory(Fitter, FitterFactory):
    ...
    def forward_tensor_infos(self, tensor_infos):
        ...
        return utils.join_dicts(
            tensor_infos, {"x_cont": TensorInfo(feat_shape=[n_cont * self.tc_num])}
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        centers = torch.linspace(
            self.tc_low, self.tc_high, self.tc_num, device=ds.device
        )[None, None, :]
...
class CircleCodingFactory(Fitter, FitterFactory):
    ...
    def forward_tensor_infos(self, tensor_infos):
        n_cont = tensor_infos["x_cont"].get_n_features()
        return utils.join_dicts(
            tensor_infos, {"x_cont": TensorInfo(feat_shape=[n_cont * 2])}
        )
...
def apply_tfms_rec(tfms: Union[BaseEstimator, List], x: torch.Tensor):
    if isinstance(tfms, list):
        return torch.stack(
            [apply_tfms_rec(tfm, x[i]) for i, tfm in enumerate(tfms)], dim=0
        )
    else:
        return torch.as_tensor(
            tfms.transform(x.detach().cpu().numpy()), dtype=x.dtype, device=x.device
        )
...
class SklearnTransformLayer(Layer):
    ...
    def _stack(self, layers):
        return SklearnTransformLayer(
            tfms=[l.tfms for l in layers], fitter=layers[0].fitter
        )
...
class SklearnTransformFactory(Fitter, FitterFactory):
    def __init__(self, tfm: BaseEstimator, **config):
        super().__init__(
            needs_tensors=True, is_individual=False, modified_tensors=["x_cont"]
        )
        self.tfm = tfm
