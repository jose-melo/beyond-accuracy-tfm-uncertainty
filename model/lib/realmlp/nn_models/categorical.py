...
class SingleEncodingFactory(FitterFactory):
    ...
    def apply_on(self, cat_size: int, n_classes: int):
        # can be overridden
        return cat_size >= self.min_cat_size and (
            self.max_cat_size < 0 or cat_size <= self.max_cat_size
        )

    def _create(self, tensor_infos):
        if "x_cat" not in tensor_infos:
            return IdentityFitter()
        x_cat_sizes = tensor_infos["x_cat"].get_cat_sizes().cpu().numpy()
...
class EncodingLayer(Layer):
    def __init__(
        self, single_enc_layers: Iterable[Layer], enc_output_name: str, fitter
    ):
    ...
    def forward_tensors(self, tensors):
        x_cat = tensors["x_cat"]
        prev_output_tensors = (
            [tensors[self.enc_output_name]] if self.enc_output_name in tensors else []
        )
        ...
        output_tensors = prev_output_tensors + [
            t["x_cont"] for t in new_tensors if "x_cont" in t
        ]
        if len(output_tensors) == 0:
            # create empty tensor
            new_conts = torch.zeros(
                *x_cat.shape[:-1], 0, device=x_cat.device, dtype=torch.float32
            )
        else:
        ...
        if len(cat_tensors) > 0:
            ...
            return utils.update_dict(
                tensors, {self.enc_output_name: new_conts, "x_cat": new_cats}
            )
        else:
            return utils.update_dict(
                tensors, {self.enc_output_name: new_conts}, remove_keys="x_cat"
            )
    ...
    def _stack(self, layers: List["EncodingLayer"]):
        return EncodingLayer(
            [
                layers[0]
                .emb_layers[i]
                .stack([layers[j].emb_layers[i] for j in range(len(layers))])
                for i in range(len(layers[0].emb_layers))
            ],
            layers[0].enc_output_name,
            layers[0].fitter,
        )
...
class EncodingFitter(Fitter):
    def __init__(
        ...
        enc_output_name: str = "x_cont",
        **config,
    ):
        super().__init__(
            needs_tensors=any([enc.needs_tensors for enc in single_encoder_fitters]),
        ...
        self.enc_output_name = (
            enc_output_name  # allow to have something other than x_cont
        )
    ...
    def get_n_params(self, tensor_infos: Dict[str, TensorInfo]) -> int:
        return sum(
            [
                f.get_n_params(ti)
                for f, ti in zip(
                    self.single_encoder_fitters, self._sub_tensor_infos(tensor_infos)
                )
            ]
        )
    ...
    def get_n_forward(self, tensor_infos: Dict[str, TensorInfo]) -> int:
        ...
        # for concat
        forward_bytes += self._get_n_values(
            self.forward_tensor_infos(tensor_infos), [self.enc_output_name, "x_cat"]
        )
    ...
    def _sub_tensor_infos(self, tensor_infos):
        ...
        if "y" in tensor_infos:
            return [
                {"x_cat": TensorInfo(cat_sizes=[cat_sz]), "y": tensor_infos["y"]}
    ...
    def forward_tensor_infos(self, tensor_infos):
        x_cat_sizes = tensor_infos["x_cat"].get_cat_sizes().cpu().numpy()
        n_cont = (
            tensor_infos[self.enc_output_name].get_n_features()
            if self.enc_output_name in tensor_infos
            else 0
        )
        out_cat_sizes = []
        ...
        if len(out_cat_sizes) > 0:
            return utils.update_dict(
                tensor_infos,
                {
                    self.enc_output_name: TensorInfo(feat_shape=[n_cont]),
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        x_cat_sizes = ds.tensor_infos["x_cat"].get_cat_sizes().cpu().numpy()
        enc_layers = []
        for i in range(len(x_cat_sizes)):
            ...
            enc_layers.append(
                enc.fit(DictDataset(tensors, tensor_infos, ds.device, ds.n_samples))
            )
...
class EncodingFactory(FitterFactory):
    ...
    def _create(self, tensor_infos):
        ...
        x_cat_sizes = tensor_infos["x_cat"].get_cat_sizes().cpu().numpy()
        single_encoder_fitters = [
            self.single_encoder_factory.create(
                {"x_cat": TensorInfo(cat_sizes=[cat_sz]), "y": tensor_infos["y"]}
            )
            for cat_sz in x_cat_sizes
        ]
        return EncodingFitter(
            single_encoder_fitters, enc_output_name=self.enc_output_name
        )
...
class SingleOneHotLayer(Layer):
    ...
    def _binary(self, x_cat, values):
        ...
        src = src[[None] * (x_cat.dim() - 1) + [slice(None)]].expand(
            *(list(x_cat.shape[:-1]) + [-1])
        )
    ...
    def _multiple(self, x_cat, on_value, off_value):
        ...
        cont = torch.full(
            cont_shape, off_value, dtype=torch.float32, device=x_cat.device
        )
        src = torch.full(
            [1] * x_cat.dim(), on_value, dtype=torch.float32, device=x_cat.device
        ).expand(*x_cat.shape)
    ...
    def forward_tensors(self, tensors):
        ...
        if self.use_missing_zero:
            if self.cat_size == 2 and self.use_1d_binary_onehot:
                ...
                cont = cont[
                    ..., 1:
                ]  # cut off the dimension for the missing value one-hot
        else:
            if self.cat_size == 2 and self.use_1d_binary_onehot:
...
class SingleOneHotFitter(Fitter):
    ...
    def forward_tensor_infos(self, tensor_infos):
        ...
        return utils.update_dict(
            tensor_infos,
            {"x_cont": TensorInfo(feat_shape=[cat_size])},
            remove_keys="x_cat",
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return SingleOneHotLayer(
            self,
            onoff=self.bin_onoff if is_binary else self.multi_onoff,
            cat_size=cat_size,
...
class SingleOneHotFactory(SingleEncodingFactory):
    def __init__(
        self,
        use_missing_zero=True,
        bin_onoff=(1.0, 0.0),
        multi_onoff=(1.0, 0.0),
        min_one_hot_cat_size=0,
        max_one_hot_cat_size=-1,
        max_one_hot_size_by_n_classes=False,
        use_1d_binary_onehot: bool = True,
        **config,
    ):
        super().__init__(
            create_fitter=lambda tensor_infos: SingleOneHotFitter(
                use_missing_zero=use_missing_zero,
                bin_onoff=bin_onoff,
                multi_onoff=multi_onoff,
                use_1d_binary_onehot=use_1d_binary_onehot,
            ),
            min_cat_size=min_one_hot_cat_size,
            max_cat_size=max_one_hot_cat_size,
        )
...
class SingleEmbeddingLayer(Layer):
    def __init__(self, emb: Variable):
        super().__init__(
            new_tensor_infos={"x_cont": TensorInfo(feat_shape=[emb.shape[-1]])},
            remove_keys="x_cat",
        )
    ...
    def forward_tensors(self, tensors):
        ...
        x_cat = x_cat.squeeze(
            -1
        )  # squeeze feature dimension, we assume that there is only one feature
        ...
        while x_cat.dim() > 1:
            ...
            parallel_idxs = torch.arange(
                x_cat.shape[-2], dtype=torch.long, device=self.emb.device
            )
...
def get_embedding_size(
    fn: Optional[Union[int, str, Callable[[int], int]]],
) -> Callable[[int], int]:
    if fn is None:
        fn = "fastai"
...
class SingleEmbeddingFitter(Fitter):
    def __init__(self, embedding_size=None, **config):
        ...
        self.size_func = (
            get_embedding_size(embedding_size)
    ...
    def forward_tensor_infos(self, tensor_infos):
        new_info = TensorInfo(
            feat_shape=[self.size_func(tensor_infos["x_cat"].get_cat_sizes()[0].item())]
        )
        return utils.update_dict(
            tensor_infos, {"x_cont": new_info}, remove_keys="x_cat"
        )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        if self.emb_init_mode == "normal":
            ...
            emb = (1.0 / np.sqrt(emb_size)) * (
                2 * torch.rand(cat_size, emb_size, device=ds.device) - 1
            )
        ...
        emb_factor = self.emb_init_gain * (
            np.sqrt(1.0 / emb_size) if self.emb_reduce_norm else 1.0
        )
        return SingleEmbeddingLayer(
            Variable(
                emb_factor * emb,
                trainable=True,
...
class SingleEmbeddingFactory(SingleEncodingFactory):
    def __init__(
        self,
        embedding_size=None,
        min_embedding_cat_size=0,
        max_embedding_cat_size=-1,
        **config,
    ):
        super().__init__(
            create_fitter=lambda tensor_infos: SingleEmbeddingFitter(
                embedding_size=embedding_size, **config
            ),
            min_cat_size=min_embedding_cat_size,
            max_cat_size=max_embedding_cat_size,
        )
...
class SingleTargetEncodingFitter(Fitter):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        y = ds.tensors["y"]
        y_cat_sizes = ds.tensor_infos["y"].get_cat_sizes().cpu().numpy()
        if y_cat_sizes[0] > 2:
...
class SingleTargetEncodingFactory(SingleEncodingFactory):
    def __init__(self, min_targetenc_cat_size=0, max_targetenc_cat_size=-1, **config):
        create_fitter = lambda tensor_infos: SingleTargetEncodingFitter(
            n_classes=tensor_infos["y"].get_cat_sizes()[0].item()
        )
        super().__init__(
            create_fitter=create_fitter,
            min_cat_size=min_targetenc_cat_size,
...
class SingleOrdinalEncodingLayer(Layer):
    def __init__(self, fitter, cat_size: int, permute_ordinal_encoding: bool = False):
        ...
        if permute_ordinal_encoding:
            self.perm = Variable(
                torch.randperm(cat_size, dtype=torch.long), trainable=False
            )
    ...
    def forward_tensors(self, tensors):
        ...
        return utils.update_dict(
            tensors, {"x_cont": x_cat.type(torch.float32)}, remove_keys="x_cat"
        )
...
class SingleOrdinalEncodingFitter(Fitter):
    ...
    def forward_tensor_infos(self, tensor_infos):
        return utils.update_dict(
            tensor_infos, {"x_cont": tensor_infos["x_cat"]}, remove_keys="x_cat"
        )

    def _fit(self, ds: DictDataset) -> Layer:
        return SingleOrdinalEncodingLayer(
            self,
            cat_size=ds.tensor_infos["x_cat"].get_cat_sizes()[0].item(),
...
class SingleOrdinalEncodingFactory(SingleEncodingFactory):
    def __init__(self, min_labelenc_cat_size=0, max_labelenc_cat_size=-1, **config):
        super().__init__(
            create_fitter=lambda tensor_infos: SingleOrdinalEncodingFitter(**config),
            min_cat_size=min_labelenc_cat_size,
            max_cat_size=max_labelenc_cat_size,
        )
