...
class ParametricActivationLayer(Layer):
    ...
    def _stack(self, layers):
        return ParametricActivationLayer(
            self.f, Variable.stack([l.weight for l in layers])
        )
...
class ParametricActivationFitter(Fitter):
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        return ParametricActivationLayer(
            self.f,
            Variable(
                torch.ones(1, n_cont, device=ds.device),
                trainable=True,
                hyper_factors={"lr": self.act_lr_factor, "wd": self.act_wd_factor},
            ),
        )
