...
from model.lib.realmlp.nn_models.base import (
    ...
    ConcatParallelFactory,
)
from model.lib.realmlp.nn_models.categorical import (
    EncodingFactory,
    SingleOneHotFactory,
    SingleEmbeddingFactory,
    SingleOrdinalEncodingFactory,
    SingleTargetEncodingFactory,
)
from model.lib.realmlp.nn_models.nn import (
    DropoutFitter,
    WeightFitter,
    BiasFitter,
    ScaleFitter,
    NoiseFitter,
    PLREmbeddingsFactory,
    ScaleFactory,
    PeriodicEmbeddingsFactory,
    RFFeatureImportanceFactory,
    LabelSmoothingFactory,
    StochasticLabelNoiseFactory,
    StochasticGateFactory,
    FeatureImportanceFactory,
    FixedWeightFactory,
    AntisymmetricInitializationFactory,
    NormalizeOutputFactory,
    ClampOutputFactory,
)
from model.lib.realmlp.nn_models.pipeline import (
    MedianCenterFactory,
    RobustScaleFactory,
    MeanCenterFactory,
    ...
    def _create_transform(self, tensor_infos):
        ...
        for layer_str in self.block_str.split("-"):
            ...
            if layer_str in ["a", "act", "activation"]:
                ...
                fitters.append(
                    WeightFitter(self.out_features, **self.config).add_scope("weight")
                )
            elif layer_str in ["b", "bias"]:
                fitters.append(
                    BiasFitter(in_features=in_features, **self.config).add_scope("bias")
                )
            ...
            #     pass  # todo: batchnorm
                ...
                out_tensor_infos = SequentialFitter(fitters).forward_tensor_infos(
                    tensor_infos
                )
                if np.equal(
                    tensor_infos["X_cont"].get_feat_shape(),
                    out_tensor_infos["X_cont"].get_feat_shape(),
                ):
...
class PreprocessingFactory(FitterFactory):
    ...
    def _create(self, tensor_infos: Dict[str, TensorInfo]) -> Fitter:
        ...
        for tfm in self.config.get("tfms", []):
            if tfm == "one_hot":
                tfm_factories.append(
                    EncodingFactory(
                        SingleOneHotFactory(**self.config), enc_output_name="x_one_hot"
                    )
                )
                tfm_factories.append(
                    RenameTensorFactory(old_name="x_one_hot", new_name="x_cont")
                )
                ...
                tfm_factories.append(
                    EncodingFactory(SingleEmbeddingFactory(**self.config)).add_scope(
                        "emb"
                    )
                )
                ...
                tfm_factories.append(
                    EncodingFactory(SingleOrdinalEncodingFactory(**self.config))
                )
            elif tfm == "target_encoding":
                tfm_factories.append(
                    EncodingFactory(SingleTargetEncodingFactory(**self.config))
                )
            elif tfm == "kdi":
                from kditransform import KDITransformer

                tfm = KDITransformer(
                    alpha=self.config.get("kdi_alpha", 1.0),
                    output_distribution=self.config.get(
                        "kdi_output_distribution", "normal"
                    ),
                )
                tfm_factories.append(SklearnTransformFactory(tfm))
            elif tfm == "quantile":
                tfm = QuantileTransformer(
                    output_distribution=self.config.get(
        ...
        if self.config.get("use_smooth_clip", False):
            tfm_factories.append(
                FunctionFactory(lambda x: x / (1 + (1 / 9) * x**2).sqrt())
            )
        ...
        return (
            SequentialFactory(tfm_factories)
            .add_scope("tfms")
            .create(tensor_infos=tensor_infos)
        )
...
class NNFactory(FitterFactory):
    ...
    def _create_transform(
        ...
        y_cat_sizes = tensor_infos["y"].get_cat_sizes().cpu().numpy()
        n_classes = y_cat_sizes[0]

        factories = []
        net_factories = []

        if "one_hot" in self.config.get("tfms", []) or self.config.get(
            "use_one_hot", False
        ):
        ...
        if num_emb_config.get("use_plr_embeddings", False):
            ...
            if num_emb_config.get("use_plr_scale", False):
                plr_factory = SequentialFactory(
                    [ScaleFactory(**num_emb_config), plr_factory]
                )
            ...
            periodic_emb_factory = PeriodicEmbeddingsFactory(
                **num_emb_config
            ).add_scope("periodic_emb")
            num_factory = SequentialFactory(
                [
                    FilterTensorsFactory(include_keys=["x_cont"]),
                    prep_factory,
        ...
        out_sizes = self.config.get("hidden_sizes", [256] * 3) + [
            len(y_cat_sizes) if n_classes == 0 else n_classes
        ]
        for i in range(len(out_sizes)):
            ...
            if i + 1 == len(out_sizes):
                config = utils.join_dicts(
                    config, {"block_str": "w-b"}, config.get("last_layer_config", {})
                )
                layer_position = "last"
            elif i == 0:
            ...
            net_factories.append(
                BlockFactory(
                    out_features=out_sizes[i], layer_position=layer_position, **config
                )
                .add_scope(block_scope)
                .add_scope(block_scope_2)
            )
        ...
        if self.config.get("use_antisymmetric_initialization", False):
            factories = [
                AntisymmetricInitializationFactory(
                    SequentialFactory(factories), **self.config
                )
            ]

        if self.config.get("output_factor", 1.0) != 1.0:
            factories.append(
                FunctionFactory(lambda x, c=self.config["output_factor"]: c * x)
            )
