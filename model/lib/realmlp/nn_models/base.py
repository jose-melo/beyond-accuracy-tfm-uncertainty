...
class TrainContext:
    ...
    def __init__(
        ...
        hp_manager: Optional[HyperparamManager] = None,
    ):
...
class Variable(ContextRecorder, nn.Parameter):
    ...
    def __init__(
        self, data=None, trainable=True, requires_grad=None, hyper_factors=None
    ):
        super().__init__()
    ...
    def stack(vars: List["Variable"], dim=0):
        ...
        with vars[0].set_context():
            with torch.no_grad():
                return Variable(
                    torch.stack(vars, dim=dim),
                    trainable=vars[0].trainable,
                    requires_grad=vars[0].requires_grad,
                    hyper_factors=vars[0].hyper_factors,
                )
...
class Layer(ContextRecorder, StringConvertible, nn.Module):
    ...
    def __init__(
        self,
        new_tensor_infos: Optional[Dict[str, TensorInfo]] = None,
        fitter: Optional["Fitter"] = None,
        remove_keys: Optional[Union[str, List[str]]] = None,
    ):
        """
    ...
    def forward_tensor_infos(
        ...
        return utils.update_dict(
            tensor_infos, self.new_tensor_infos, remove_keys=self.remove_keys
        )

    def forward(
        self, data: Union[DictDataset, Dict[str, torch.Tensor]]
    ...
    def forward_ds(self, ds: DictDataset) -> DictDataset:
        ...
        return DictDataset(
            None if ds.tensors is None else self.forward_tensors(ds.tensors),
            self.forward_tensor_infos(ds.tensor_infos),
            device=ds.device,
    ...
    def forward_tensors(
        ...
        return utils.join_dicts(
            tensors, {"x_cont": self.forward_cont(tensors["x_cont"])}
        )
...
class SequentialLayer(Layer):
    def __init__(self, tfms: List[Layer]):
        ...
        self.tfms = nn.ModuleList(
            [tfm for tfm in tfms if not isinstance(tfm, IdentityLayer)]
        )
    ...
    def _stack(self, seq_tfms):
        return SequentialLayer(
            [
                seq_tfms[0].tfms[i].stack([seq_tfm.tfms[i] for seq_tfm in seq_tfms])
    ...
    def __str__(self):
        sub_strings = [
            "  " + line for tfm in self.tfms for line in str(tfm).split("\n")
        ]
...
class ResidualLayer(Layer):
    ...
    def _stack(self, seq_tfms):
        return ResidualLayer(
            seq_tfms[0].inner_layer.stack([seq_tfm.inner_layer for seq_tfm in seq_tfms])
        )
...
class ConcatParallelLayer(Layer):
    ...
    def forward_tensors(self, tensors):
        ...
        return {
            key: torch_utils.cat_if_necessary(
                [t[key] for t in out_tensors if key in t], dim=-1
            )
            for key in out_keys
        }

    def _stack(self, tfms: List[Layer]):
        return ConcatParallelLayer(
            [
                tfms[0].layers[i].stack([tfm.layers[i] for tfm in tfms])
                for i in range(len(tfms[0].layers))
            ],
            fitter=tfms[0].fitter,
        )
    ...
    def __str__(self):
        sub_strings = [
            "  " + line for tfm in self.layers for line in str(tfm).split("\n")
        ]
        return f"{self.__class__.__name__} [\n" + "\n".join(sub_strings) + "\n]\n"
...
class FilterTensorsLayer(Layer):
    ...
    def forward_tensors(
        ...
        result = {
            key: (
                value
                if (self.include_keys is None or key in self.include_keys)
...
class BiasLayer(Layer):
    ...
    def _stack(self, tfms):
        return BiasLayer(
            Variable.stack([tfm.bias for tfm in tfms]), factor=tfms[0].factor
        )
...
class WeightLayer(Layer):
    def __init__(self, weight: Variable, factor: float = 1.0):
        super().__init__(
            new_tensor_infos={"x_cont": TensorInfo(feat_shape=[weight.shape[-1]])}
        )
    ...
    def _stack(self, tfms):
        return WeightLayer(
            Variable.stack([tfm.weight for tfm in tfms]), factor=tfms[0].factor
        )
...
class RenameTensorLayer(Layer):
    ...
    def forward_tensors(
        ...
        if self.old_name not in tensors:
            ...
            return utils.update_dict(
                tensors,
                {self.new_name: tensors[self.old_name]},
                remove_keys=self.old_name,
            )
        else:
            # print(f'{tensors[self.new_name].shape=}, {tensors[self.old_name].shape=}')
            new_tensor = torch.cat(
                [tensors[self.new_name], tensors[self.old_name]], dim=-1
            )
            return utils.update_dict(
                tensors, {self.new_name: new_tensor}, remove_keys=self.old_name
            )
...
class Fitter(ContextAware, StringConvertible):
    ...
    def _get_n_values(
        self, tensor_infos: Dict[str, TensorInfo], relevant_tensors: Optional[List[str]]
    ):
        """
    ...
    def get_n_forward(self, tensor_infos: Dict[str, TensorInfo]) -> int:
        ...
        return self._get_n_values(
            self.forward_tensor_infos(tensor_infos), self.modified_tensors
        )
    ...
    def fit_transform(
        self, ds: DictDataset, needs_tensors: bool = True
    ...
    def fit_transform_subsample(
        self, ds: DictDataset, ram_limit_gb: float, needs_tensors: bool = True
    ) -> Tuple[Layer, DictDataset]:
        """
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        ...
        if self.__class__._fit_transform != Fitter._fit_transform:
            ...
            # avoid infinite recursion if the method is not overridden
            tfm, ds = self._fit_transform_subsample(
                ds, ram_limit_gb=np.Inf, needs_tensors=False
            )
            return tfm
    ...
    def _fit_transform(
        self, ds: DictDataset, needs_tensors: bool
    ) -> Tuple[Layer, DictDataset]:
        """
        ...
        if self.__class__._fit_transform_subsample != Fitter._fit_transform_subsample:
            return self._fit_transform_subsample(
                ds, ram_limit_gb=np.Inf, needs_tensors=needs_tensors
            )
        else:
            tfm = self._fit(ds)
            if needs_tensors:
                ...
                return tfm, DictDataset(
                    None,
                    tfm.forward_tensor_infos(ds.tensor_infos),
                    ds.device,
                    ds.n_samples,
                )

    def _fit_transform_subsample(
        self, ds: DictDataset, ram_limit_gb: float, needs_tensors: bool = True
    ) -> Tuple[Layer, DictDataset]:
        ...
        if (
            ram_limit_gb < np.Inf
            and n_forward > 0
            and ds.tensors is not None
            and (self.needs_tensors or needs_tensors)
        ):
            ...
            if max_n_samples < ds.n_samples:
                ...
                subsample_idxs = torch.randperm(ds.n_samples, device=ds.device)[
                    :max_n_samples
                ]
...
class SequentialFitter(Fitter):
    def __init__(self, fitters: List[Fitter], **config):
        super().__init__(
            needs_tensors=np.any([f.needs_tensors for f in fitters]),
    ...
    def _fit_transform(self, ds: DictDataset, needs_tensors: bool = True):
        ...
        max_tensors_idx = (
            np.max(np.argwhere(needs_tensors_list)) if np.any(needs_tensors_list) else 0
        )
        tfms = []
    ...
    def _fit_transform_subsample(
        ...
        tfms = []
        for i, fitter in enumerate(self.fitters):
            tfm, ds = fitter.fit_transform_subsample(
                ds, ram_limit_gb=ram_limit_gb, needs_tensors=(i < max_tensors_idx)
            )
            tfms.append(tfm)
        return SequentialLayer(tfms), ds
    ...
    def split_off_dynamic(self):
        ...
        if np.any(is_dynamic):
            ...
            return SequentialFitter(
                self.fitters[:first_dynamic] + [static]
            ).add_others_scope(self), SequentialFitter(
                [dynamic] + self.fitters[first_dynamic + 1 :]
            ).add_others_scope(
                self
            )
        else:
    ...
    def split_off_individual(self):
        ...
        if np.any(is_individual):
            ...
            return SequentialFitter(
                self.fitters[:first_indiv] + [non_indiv]
            ).add_others_scope(self), SequentialFitter(
                [indiv] + self.fitters[first_indiv + 1 :]
            ).add_others_scope(
                self
            )
        else:
    ...
    def __str__(self):
        sub_strings = [
            "  " + line for fitter in self.fitters for line in str(fitter).split("\n")
        ]
        return f"{self.__class__.__name__} [\n" + "\n".join(sub_strings) + "\n]\n"
...
class ResidualFitter(Fitter):
    def __init__(self, inner_fitter: Fitter, **config):
        super().__init__(
            needs_tensors=inner_fitter.needs_tensors,
    ...
    def get_n_forward(self, tensor_infos: Dict[str, TensorInfo]):
        return self.inner_fitter.get_n_forward(tensor_infos) + self._get_n_values(
            tensor_infos, ["x_cont"]
        )
    ...
    def __str__(self):
        sub_strings = [
            "  " + line
            for fitter in [self.inner_fitter]
            for line in str(fitter).split("\n")
        ]
        return f"{self.__class__.__name__} [\n" + "\n".join(sub_strings) + "\n]\n"
...
class ConcatParallelFitter(Fitter):
    ...
    def get_n_forward(self, tensor_infos: Dict[str, TensorInfo]) -> int:
        ...
        concat_space = self._get_n_values(
            out_tensor_infos, relevant_tensors=list(out_tensor_infos.keys())
        )
    ...
    def forward_tensor_infos(
        ...
        out_tensor_infos_list = [
            f.forward_tensor_infos(tensor_infos) for f in self.fitters
        ]
        out_keys = {key for ti in out_tensor_infos_list for key in ti.keys()}
        return {
            key: TensorInfo(
                cat_sizes=torch_utils.cat_if_necessary(
                    [
                        ti[key].get_cat_sizes()
...
class ConcatParallelFactory(FitterFactory):
    ...
    def _create(self, tensor_infos) -> Fitter:
        return ConcatParallelFitter(
            [factory.create(tensor_infos) for factory in self.factories]
        )
...
class FilterTensorsFactory(Fitter, FitterFactory):
    ...
    def forward_tensor_infos(
        ...
        return {
            key: (
                ti
                if (
                    (self.include_keys is None or key in self.include_keys)
                    and (self.exclude_keys is None or key not in self.exclude_keys)
                )
    ...
    def _fit(self, ds: DictDataset) -> Layer:
        return FilterTensorsLayer(
            include_keys=self.include_keys, exclude_keys=self.exclude_keys, fitter=self
        )
...
class RenameTensorFactory(Fitter, FitterFactory):
    ...
    def forward_tensor_infos(
        ...
        if self.old_name not in tensor_infos:
            ...
            return utils.update_dict(
                tensor_infos,
                {self.new_name: tensor_infos[self.old_name]},
                remove_keys=self.old_name,
            )
        else:
            # both names exist in tensor_infos
            new_tensor_info = TensorInfo.concat(
                [tensor_infos[self.new_name], tensor_infos[self.old_name]]
            )
            return utils.update_dict(
                tensor_infos,
                {self.new_name: new_tensor_info},
                remove_keys=self.old_name,
            )

    def _fit(self, ds: DictDataset) -> Layer:
        return RenameTensorLayer(
            old_name=self.old_name, new_name=self.new_name, fitter=self
        )
