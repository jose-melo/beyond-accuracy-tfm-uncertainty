    ...
    def __init__(self, opt, hyper_mappings, hp_manager: HyperparamManager):
        ...
        for names, opt_name, defaults in hyper_mappings:
            ...
            for name, default in zip(names, defaults):
                self.hyper_getters[name] = [
                    self.hp_manager.register_hyper(
                        name, group["params"][0].context.scope, default=default
                    )
        ...
        for names, opt_name, defaults in self.hyper_mappings:
            ...
            if isinstance(names, tuple):
                for i, group in enumerate(self.opt.param_groups):
                    group[opt_name] = tuple(
                        self.get_hyper_values(name, i) for name in names
                    )
                        ...
                        if wd != 0.0:
                            for p in group["params"]:
                                p.mul_(
                                    ...
                                    - wd
                                    * lr
                                    * p.hyper_factors.get("wd", 1.0)
                                    * p.hyper_factors.get("lr", 1.0)
                                )

            else:
                raise RuntimeError(
                    "Could not understand mapping {}".format(
                        (names, opt_name, defaults)
                    )
                )
...
class AMSGradOptimizer(OptimizerBase):
    def __init__(self, param_groups, hp_manager):
        super().__init__(
            optim.Adam(param_groups, amsgrad=True),
            hyper_mappings=[
                ("lr", "lr", 1e-3),
                (("mom", "sq_mom"), "betas", (0.9, 0.999)),
                ("opt_eps", "eps", 1e-8),
