...
class HyperparamManager:
    class HyperGetter:
        ...
        def __call__(self):
            return (
                self.tc.hyper_base_values[self.hyper_name][self.base_value_pattern]
    ...
    def __init__(self, **config):
        ...
        self.reg_terms = []
        self.needs_update = (
            True  # indicates whether self.hyper_sched_values needs to be updated
        )
    ...
    def register_hyper(
        ...
        scope,
        default=None,
        default_sched=lambda: ConstantSchedule(1.0),
    ):
        if name not in self.hyper_scheds:
            ...
            sched_dict = {
                key: get_schedule(sched) if isinstance(sched, str) else sched()
        ...
        return HyperparamManager.HyperGetter(
            self,
            name,
    ...
    def update_hyper_sched_values(self):
        if self.needs_update:
            # print(f'update')
            self.hyper_sched_values = {
                name: {
                    pattern: sched.get_value() for pattern, sched in sched_dict.items()
                }
