...
class Metric:
    ...
    def apply_dict(self, metrics_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            key: value for key, value in result_dict.items() if key not in metrics_dict
        }
...
class TorchMetric(Metric):
    def apply_torch(
        self, y_pred: torch.Tensor, y: torch.Tensor, reduction: Optional[str] = "mean"
    ) -> torch.Tensor:
        raise NotImplementedError()
...
def zip_dicts(list_of_dicts):
    return {
        key: [list_of_dicts[i][key] for i in range(len(list_of_dicts))]
...
def unzip_dict(dict_of_lists: dict):
    ...
    return [
        {key: dict_of_lists[key][i] for i in range(list_length)}
...
def mse(y_pred, y, reduction="mean"):
    ...
    if y_pred.dim() != y.dim():
        raise RuntimeError(
            "MSE: y_pred.dim() != y.dim(): could lead to broadcasting errors"
        )
...
def pinball_loss(
    y_pred: torch.Tensor, y: torch.Tensor, quantile: float, reduction="mean"
):
    if y_pred.dim() != y.dim():
        raise RuntimeError(
            "Pinball loss: y_pred.dim() != y.dim(): could lead to broadcasting errors"
        )
    err = y_pred - y
...
def mean_interleave(input, repeats, dim):
    ...
    new_shape = (
        input.shape[:dim]
        + [input.shape[dim] // repeats, repeats]
        + input.shape[dim + 1 :]
    )
...
def get_y_probs(y: torch.Tensor, n_classes: int) -> torch.Tensor:
    ...
    if len(y.shape) > 2:
        # recursion
        return cat_if_necessary(
            [get_y_probs(y[i], n_classes) for i in range(y.shape[0])], dim=0
        )

    return (
        torch.bincount(y.squeeze(-1), minlength=n_classes).to(torch.float32)
        / y.shape[0]
    )


def insert_missing_class_columns(
    y_pred: torch.Tensor, train_ds: DictDataset
) -> torch.Tensor:
    """
    ...
    train_class_counts = torch.bincount(
        train_ds.tensors["y"].squeeze(-1), minlength=n_classes
    ).cpu()
    ...
    missing_values = logsumexp + np.log(
        posterior_prob / (1 - posterior_prob * n_missing)
    )
...
def remove_missing_classes(
    ...
    class_mapping[is_present] = torch.arange(
        num_present, dtype=torch.long, device=y.device
    )
...
def expected_calibration_error(y_pred: torch.Tensor, y: torch.Tensor):
    ...
    for y_pred_indiv, y_indiv in zip(y_pred_models, y_models):
        ...
        y_pred_indiv_probs = y_pred_indiv_probs / y_pred_indiv_probs.sum(
            dim=-1, keepdim=True
        )
        ...
        metric = torchmetrics.CalibrationError(
            task="binary" if is_binary else "multiclass", num_classes=num_classes
        )
...
class Metrics:
    ...
    def compute_metrics_dict(
        self, y_preds: List[torch.Tensor], y: torch.Tensor, use_ens: bool
    ) -> NestedDict:
        """
    ...
    def compute_val_score(self, val_metrics_dict: NestedDict) -> float:
        ...
        individual_val_scores = [
            indiv_dict[self.val_metric_name]
            for indiv_dict in val_metrics_dict["1"].values()
        ]
    ...
    def apply(y_pred: torch.Tensor, y: torch.Tensor, metric_name: str) -> torch.Tensor:
        ...
        if torch.any(invalid):
            if y.is_floating_point():
                ...
                if len(not_invalid) == 0:
                    ...
                    y_pred[invalid] = (
                        torch.min(not_invalid) - 100
                    )  # a very small value, basically zero probability
        ...
        if metric_name == "class_error":
            return (
                torch.count_nonzero(
                    y_pred.argmax(dim=-1) != get_y_categorical(), dim=-1
                )
                / y_pred.shape[-2]
            )
            ...
            return 1.0 - Metrics.apply_sklearn_classification_metric(
                y_pred,
                y,
                lambda y1, y2: roc_auc_score(y1, y2, multi_class="ovo"),
                needs_pred_probs=True,
            )
        elif metric_name == "1-auc_ovr":
            return 1.0 - Metrics.apply_sklearn_classification_metric(
                y_pred,
                y,
                lambda y1, y2: roc_auc_score(y1, y2, multi_class="ovr"),
                needs_pred_probs=True,
            )
            ...
            return 1.0 - Metrics.apply_sklearn_classification_metric(
                y_pred, y, balanced_accuracy_score, needs_pred_probs=False
            )
        elif metric_name == "1-mcc":
            return 1.0 - Metrics.apply_sklearn_classification_metric(
                y_pred, y, matthews_corrcoef, needs_pred_probs=False
            )
            ...
            best_constant_y_pred = torch_np_quantile(
                y, quantile, dim=-2, keepdim=True
            ).expand(*y_pred.shape)
    ...
    @staticmethod
    def apply_sklearn_classification_metric(
        y_pred: torch.Tensor,
        y: torch.Tensor,
        metric_function: Callable,
        needs_pred_probs: bool,
        two_class_single_column: bool = True,
    ):
        if y.is_floating_point():
            y = y.argmax(dim=-1)
        else:
    ...
    def defaults(y_cat_sizes, val_metric_name: Optional[str] = None) -> "Metrics":
        ...
        if len(y_cat_sizes) == 1 and y_cat_sizes[0] == 2:
            # bin class
            return Metrics(
                default_class_metrics, val_metric_name, TaskType.CLASSIFICATION
            )
        elif y_cat_sizes[0] > 0:
            if y_cat_sizes[0] > 100:
                default_class_metrics = [
                    m for m in default_class_metrics if m != "1-auc_ovo"
                ]
            ...
            return Metrics(
                ["rmse", "mae", "max_error", "nrmse", "nmae", "n_max_error"],
                val_metric_name,
                TaskType.REGRESSION,
            )
