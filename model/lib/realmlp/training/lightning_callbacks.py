...
class ParamCheckpointer:
    ...
    def save(self, parallel_idx: int, model_idx: int, model: Layer):
        ...
        with torch.no_grad():
            for ckpt, values in [
                (self.ckpt_params, model.parameters()),
                (self.ckpt_buffers, model.buffers()),
            ]:
                if ckpt[idx] is None:
...
class L1L2RegCallback(Callback):
    def __init__(self, hp_manager: HyperparamManager, model: Layer):
        ...
        self.l1_getters = [
            self.hp_manager.register_hyper("l1_reg", p.context.scope, default=0.0)
            for p in self.params
        ]
        self.l2_getters = [
            self.hp_manager.register_hyper("l2_reg", p.context.scope, default=0.0)
            for p in self.params
        ]
    ...
    def on_after_backward(
        ...
        for l1_getter, l2_getter, p in zip(
            self.l1_getters, self.l2_getters, self.params
        ):
...
class ModelCheckpointCallback(Callback):
    def __init__(
        ...
        self.ckpt = ParamCheckpointer(
            n_tv_splits=n_tv_splits, n_tt_splits=self.n_tt_splits
        )
        ...
        for tt_split_idx in range(self.n_tt_splits):
            for tv_split_idx in range(self.n_tv_splits):
                if self.use_best_mean_epoch:
                    if (
                        pl_module.best_mean_val_epochs[tt_split_idx]
                        == pl_module.progress.epoch
                    ):
                        # if this is the best epoch, save the model
                        self.ckpt.save(tt_split_idx, tv_split_idx, pl_module.model)
                else:
                    if (
                        pl_module.best_val_epochs[tt_split_idx][tv_split_idx]
                        == pl_module.progress.epoch
                    ):
    ...
    def on_fit_end(
        ...
        if not self.restore_best:
            raise RuntimeError(
                "ValidationCallback: Cannot restore best params when using save_best_params=False"
            )
...
class StopAtEpochsCallback(Callback):
    def __init__(
        ...
        self.model = model
        self.ckpt = ParamCheckpointer(
            n_tv_splits=n_models, n_tt_splits=len(stop_epochs)
        )
        self.logger = logger
