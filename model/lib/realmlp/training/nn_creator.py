...
from model.lib.realmlp.data.data import (
    DictDataset,
...
from model.lib.realmlp.training.lightning_callbacks import (
    StopAtEpochsCallback,
    HyperparamCallback,
    L1L2RegCallback,
...
class NNCreator:
    ...
    def setup_from_dataset(
        ...
        assert np.all(
            [
                (split_idxs.val_idxs is not None) == self.is_cv
                for split_idxs in idxs_list
            ]
        )
        ...
        self.train_idxs = torch.cat(
            [split_idxs.train_idxs for split_idxs in idxs_list], dim=0
        )
        self.val_idxs = (
            torch.cat([split_idxs.val_idxs for split_idxs in idxs_list], dim=0)
            if self.is_cv
            else None
        )
    ...
    def get_criterions(self) -> Tuple[Callable, str]:
        task_type = (
            TaskType.REGRESSION if self.n_classes == 0 else TaskType.CLASSIFICATION
        )
        # train criterion
        ...
        if train_metric_name is None:
            ...
            train_criterion = lambda y_pred, y, mn=train_metric_name: Metrics.apply(
                y_pred, y, mn
            )
        # else:
        #     raise ValueError(f'{train_metric_name=} is currently not supported')

        val_criterion = self.config.get(
            "val_metric_name", Metrics.default_metric_name(task_type)
        )
    ...
    def create_model(self, ds: DictDataset, idxs_list: List[SplitIdxs]):
        ...
        for split_idx, split_idxs in enumerate(idxs_list):
            ...
            with torch.no_grad():
                ...
                for sub_idx in range(split_idxs.n_trainval_splits):
                    ...
                    data_fitter, individual_fitter = (
                        dynamic_fitter.split_off_individual()
                    )
                    ram_limit_gb = self.config.get("init_ram_limit_gb", 1.0)
                    with set_hp_context(self.hp_manager):
                        torch.manual_seed(
                            split_idxs.split_seed
                        )  # should not be necessary, but just in case
                        data_tfm, tfmd_ds = data_fitter.fit_transform_subsample(
                            train_ds,
                            ram_limit_gb,
                            needs_tensors=individual_fitter.needs_tensors,
                        )

                    torch.manual_seed(split_idxs.sub_split_seeds[sub_idx])
                    with set_hp_context(self.hp_manager):
                        individual_tfm = individual_fitter.fit_transform_subsample(
                            tfmd_ds, ram_limit_gb=ram_limit_gb, needs_tensors=False
                        )[0]
                    if is_single_split and self.config.get(
                        "allow_single_split_opt", True
                    ):
                        self.static_model = SequentialLayer(
                            [self.static_model, data_tfm]
                        )
        ...
        fixed_init_params: Optional[List[Variable]] = self.config.get(
            "fixed_init_params", None
        )
        if fixed_init_params is not None:
            ...
            for param, fixed_init_param in zip(
                vectorized_model.parameters(), fixed_init_params
            ):
                ...
                if any(pattern in scope_str for pattern in fixed_init_param_patterns):
                    ...
                    with torch.no_grad():
                        ...
                        param.hyper_factors["lr"] = (
                            reinit_lr_factor
                            * fixed_init_param.hyper_factors.get("lr", 1.0)
                        )
    ...
    def create_callbacks(self, model: Layer, logger: Logger):
        callbacks = [
            HyperparamCallback(self.hp_manager),
            L1L2RegCallback(self.hp_manager, model),
        ]
        # if validation
        if (
            self.is_cv
            and self.fit_params is None
            and self.config.get("use_best_epoch", True)
        ):
            callbacks.append(
                ModelCheckpointCallback(
                    self.n_tt_splits,
                    n_tv_splits=self.n_tv_splits,
                    use_best_mean_epoch=self.config.get(
                        "use_best_mean_epoch_for_cv", False
                    ),
            ...
            if self.config.get("use_best_mean_epoch_for_refit", True):
                stop_epochs = [
                    [params["stop_epoch"]] * self.n_tv_splits
                    for params in self.fit_params
                ]
            else:
                if (
                    "best_indiv_stop_epochs" not in self.fit_params[0]
                    or len(self.fit_params[0]["best_indiv_stop_epochs"])
                    != self.n_tv_splits
                ):
                    raise ValueError(
                        f"Setting use_best_mean_epoch_for_refit=False "
                        f"requires setting use_best_epoch=True and n_cv==n_refit"
                    )
                stop_epochs = [
                    params["best_indiv_stop_epochs"] for params in self.fit_params
                ]
            callbacks.append(
                StopAtEpochsCallback(
                    stop_epochs=stop_epochs,
                    n_models=self.n_tv_splits,
                    model=model,
                    logger=logger,
                )
            )
    ...
    def create_dataloaders(self, ds: DictDataset):
        ...
        train_dl = ParallelDictDataLoader(
            ...
            shuffle=True,
            drop_last=True,
            adjust_bs=self.config.get("adjust_bs", False),
        )
        val_dl = None
        if self.is_cv and self.fit_params is None:
            val_dl = ValDictDataLoader(
                ds,
                self.val_idxs,
                val_batch_size=self.config.get("predict_batch_size", 1024),
            )
