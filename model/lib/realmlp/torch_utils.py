...
def batch_randperm(n_batch, n, device="cpu"):
    ...
    return torch.stack(
        [torch.randperm(n, device=device) for i in range(n_batch)], dim=0
    )
...
def hash_tensor(tensor: torch.Tensor) -> int:
    ...
    # the .cpu().numpy() appears to be necessary for equal tensors to have equal hashes
    return hash(pickle.dumps(tensor.detach().cpu().numpy()))


def torch_np_quantile(
    tensor: torch.Tensor, q: float, dim: int, keepdim: bool = False
) -> torch.Tensor:
    """
