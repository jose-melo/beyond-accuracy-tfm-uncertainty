...
class AlgInterfaceEstimator(BaseEstimator):
    ...
    def fit(
        ...
        X,
        y,
        val_idxs: Optional[np.ndarray] = None,
        cat_features: Optional[Union[List[bool], np.ndarray]] = None,
    ) -> BaseEstimator:
        """
        Fit the estimator.
        ...
        # print(f'{X=}')
        # print(f'{y=}')
        check_X_y(X, y, ensure_all_finite="allow-nan", multi_output=True)
        X = to_normal_type(X)
        y = to_normal_type(
            y
        )  # need to convert array-like objects to arrays for self.is_y_1d_

        if self._is_classification():
            # classes_ is overridden later, but this raises an error when y is a regression target, so it is useful
            self.classes_ = unique_labels(y)

        self.is_y_1d_ = isinstance(y, pd.Series) or (
        ...
        if torch.any(torch.isnan(x_ds.tensors["x_cont"])):
            raise ValueError(
                "NaN values in continuous columns are currently not allowed!"
            )
        ...
        if self._is_classification():
            ...
            y_ds = DictDataset(
                tensors={"y": torch.as_tensor(y_tfmd, dtype=torch.long)},
            ...
            if not self._supports_single_class() and len(self.classes_) == 1:
                raise ValueError(
                    f"Training with only one class in the training set is not supported!"
                )
        else:
            # regression
            ...
            if len(y_tfmd.shape) != 2:
                raise ValueError("len(y.shape) != 2")
            y_ds = DictDataset(
                tensors={"y": torch.as_tensor(y_tfmd, dtype=torch.float32)},
            ...
            if (
                not self._supports_multioutput()
                and y_ds.tensor_infos["y"].get_n_features() > 1
            ):
                raise ValueError(
        ...
        self.n_features_in_ = (
            ds.tensor_infos["x_cont"].get_n_features()
            + ds.tensor_infos["x_cat"].get_n_features()
        )
        ...
        if isinstance(random_state, int):
            ...
            raise ValueError(
                f"random_state type {type(random_state)} "
                f"is not one of [NoneType, int, np.random.RandomState]"
            )

        split_seed = seed
        refit_split_seed = seed + 1
        sub_split_seeds = list(
            np.random.RandomState(split_seed).randint(0, 2**31 - 1, size=n_cv)
        )
        ...
        if val_idxs is not None:
            ...
            if val_idxs.shape[0] == 1 and n_cv > 1:
                ...
                raise ValueError(
                    f"Value provided for {n_cv=} is not equal to {val_idxs.shape[0]=}"
                )
        else:
            if n_cv == 1:
                # random split
                splitter = RandomSplitter(
                    seed=split_seed, first_fraction=1.0 - val_fraction
                )
                ...
                splitter = KFoldSplitter(
                    k=n_cv, seed=split_seed, stratified=self._is_classification()
                )
        ...
        idxs_list = [
            SplitIdxs(
                ...
                sub_split_seeds=sub_split_seeds,
                split_id=0,
            )
        ]
        ...
        if device is None:
            allowed_device_names = [
                ...
                if name.split(":")[0] in self._allowed_device_names()
            ]
            ...
            if device not in device_names:
                raise ValueError(
                    f'Unknown device name "{device}", known device names are {device_names}'
                )
        ...
        interface_resources = InterfaceResources(
            n_threads=n_threads, gpu_devices=gpu_devices
        )
        self.cv_alg_interface_.fit(
            ds=ds,
            idxs_list=idxs_list,
        ...
        if n_refit > 0:
            self.refit_alg_interface_ = self.cv_alg_interface_.get_refit_interface(
                n_refit=n_refit
            )
            train_idxs = torch.arange(ds.n_samples, dtype=torch.long)[None, :].expand(
                n_refit, -1
            )
            refit_idxs_list = [
                SplitIdxs(
                    ...
                    val_idxs=None,
                    test_idxs=None,
                    split_seed=refit_split_seed,
                    sub_split_seeds=refit_sub_split_seeds,
                    split_id=0,
                )
            ]
            self.refit_alg_interface_.fit(
                ds=ds,
                idxs_list=refit_idxs_list,
                interface_resources=interface_resources,
                logger=logger,
                tmp_folders=refit_tmp_folders,
    ...
    def _predict_raw(self, X) -> torch.Tensor:
        ...
        check_array(X, ensure_all_finite="allow-nan")
...
class AlgInterfaceRegressor(AlgInterfaceEstimator, RegressorMixin):
    ...
    def predict(self, X):
        y_preds = self._predict_raw(X)
        y_np = y_preds.mean(dim=0).cpu().numpy()
        if self.is_y_1d_:
            y_np = y_np[:, 0]
