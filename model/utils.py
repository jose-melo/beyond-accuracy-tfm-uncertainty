...
def load_config(args, config=None, config_name=None):
    ...
    if config is None:
        config_path = os.path.join(
            ...
            args.dataset,
            "{}.json".format(
                args.model_type if args.config_name is None else args.config_name
            ),
        )
...
def get_classical_args():
    ...
    with open("configs/classical_configs.json", "r") as f:
    ...
    with pkg_resources.files(TALENT).joinpath(
        "configs/default", args.model_type + ".json"
    ).open("r") as f:
        default_para = json.load(f)

    with pkg_resources.files(TALENT).joinpath(
        "configs/opt_space", args.model_type + ".json"
    ).open("r") as f:
...
def get_deep_args():
    ...
    warnings.filterwarnings("ignore")
    with pkg_resources.files(TALENT).joinpath("configs/deep_configs.json").open(
        "r"
    ) as f:
        default_args = json.load(f)
    parser.add_argument("--dataset", type=str, default=default_args["dataset"])
...
def tune_hyper_parameters(args, opt_space, train_val_data, info):
    ...
    if (
        osp.exists(osp.join(args.save_path, "{}-tuned.json".format(args.model_type)))
        and args.retune == False
    ):
        with open(
            osp.join(args.save_path, "{}-tuned.json".format(args.model_type)), "rb"
        ) as fp:
        ...
        with open(
            osp.join(args.save_path, "{}-tuned.json".format(args.model_type)), "w"
        ) as fp:
...
def get_method(model):
    ...
    if model == "mlp":
        ...
        from model.methods.tabautopnpnet import TabAutoPNPNetMethod

        return TabAutoPNPNetMethod
    elif model == "mitra":
        from model.methods.mitra import MitraMethod
